...
-- 남았는지 큐가 안다.
--
-- 왜 `analysis_documents` 에 한 덩어리로 넣지 않는가. AC2.5 의 검증 방법이
-- 「데이터 모델 X 를 사용하는 feature 전부」라는 **행 선택 질의**다. JSON blob 으로
-- 두면 그 질의가 모든 분석의 blob 을 파싱해야 답이 나온다 — 0007 이 후보를 행으로
-- 고른 것과 같은 이유이며, 「단순 텍스트 문서가 아니라 구조화된 데이터」라는 AC2.5
-- 의 문면 그대로다.
--
...
--
-- `category` 는 AC2.4 가 열거한 7종이다. CHECK 로 못박는 이유는 이 목록이
-- 화면의 분류 칩·프롬프트·역방향 질의의 축을 동시에 이루기 때문이다 — 오타 하나가
-- 조용히 8번째 분류를 만들면 「전체 7종」이 거짓이 된다. 값의 단일 정의는
-- `backend/src/dependencies.rs` 의 `CATEGORIES` 이고 이 제약은 그 두 번째 방어선이다.
--
-- `evidence` 가 NULL 인 것은 결함이 아니라 **기록된 사실**이다. 여정
-- `JRN-review-feature` 의 예외 표가 「의존성 근거가 비어 있음 → "근거 없음"으로 명시,
-- 임의로 채우지 않음」을 요구한다. 근거를 지어내 채우는 것보다 비어 있는 편이 옳다.
--
-- `model` 과 토큰 수를 요청 행에 남기는 것은 다른 단계가 `analysis_documents` 에
-- 남기는 것과 같은 습관이다(AC4.6 의 실측 비용 회계는 아직 이 값을 읽지 않는다).
...
-- AC2.5 의 역방향 질의("이 데이터 모델을 쓰는 feature 전부")가 고르는 축.
