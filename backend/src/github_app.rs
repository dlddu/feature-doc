...
//!
//! Signs an App JWT with the App private key and calls whatever `GITHUB_API_BASE`
//! points at — real GitHub in production, or the mock server ([`crate::mock_github`])
//! in tests and the kind e2e. Error messages never include the JWT, the token, or
//! the key (AC4.3).
...
/// link it. The Setup URL's installation_id is spoofable (GitHub docs), so we list
/// the user's installations with their stored OAuth token and require a match.
