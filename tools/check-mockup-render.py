...
#    중 「구현 쪽이 식인」 경우는 여전히 사람 몫이다. 보류 건수를 요약에 찍는 이유가 그것이다:
#    0 이 아니면 그만큼이 기계 대조 밖이다.
...
#    접근 해제 통지가 그렇게 숨었다 — 원장 등재로 닫았다). **요소 축은 M11 이 닫았고 남은 것은
#    조건 축**이다: 한 요소가 목업에 없는 *조건*으로 서는지는 여전히 세지 않고, 세려면 먼저
...
#    묻고 **자식의 유무는 묻지 않는다**. 그 자식 축은 M12 가 닫았고, 닫히지 않은 것은 **짝이 없는
#    목업 요소**다: 구현이 그 요소를 아예 그리지 않으면 공유 키가 생기지 않아 M12 의 분모에도
#    들어오지 않는다(그쪽은 활성 승격이나 원장 등재로만 닫힌다).
...
def element_body(src: str, tag: str, at: int) -> str:
    """같은 이름의 자손이 있으므로 깊이를 센다 — 첫 `</name>` 에서 끊으면 중첩된
    요소의 문면을 놓친다."""
    name = tag_name(tag)
...
def same_state(mockup: list[str], impl: list[str]) -> bool:
    """**임의 임계를 쓰지 않는다** — 넣으면 「같은 자리인가」가 조율 가능한 값이 된다."""
...
def main() -> int:
    ...
    # 순서를 뒤집지 말 것 — 먼저 「같은 상태인가」를 확정하고 확정된 쌍에만 클래스를
    # 묻는다. 거꾸로 하면 키가 충돌한 쌍의 클래스 차이를 이탈로 오진한다.
    ...
    # 분모를 요소 본문으로 좁히지도 `frontend/src` 전체로 넓히지도 말 것 — 좁히면 구현이
    # 형제·부모로 옮겨 그린 문면이 거짓 양성이 되고, 넓히면 남의 화면 카피가 이 자리의
    # 공백을 가린다.
