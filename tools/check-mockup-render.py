...
#  M12 짝 안의 카피  짝으로 확정된(M10 이 「같은 자리」로 센) 쌍마다, 목업 쪽 요소의
#                   **자기 카피**(예시값·변이 제외)가 구현 쪽 요소를 담은 파일에 있어야
#                   한다. 분모가 단계가 아니라 **요소**라 활성 단계 승격과 무관하다 —
#                   M3A 가 보지 않는 단계의 상태 블록도 짝만 있으면 카피 축에 들어온다.
#
# ── 이 게이트가 보지 않는 것(의도적) ─────────────────────────────────────
...
#  * **M3A 의 분모는 「활성 단계」다.** 활성으로 승격되지 않은 단계 안의 카피는 한 건도 세지
#    않으므로, 그 단계의 상태 블록을 구현이 `data-testid` 로 짝지어 두어도 카피 축은 그 자리를
#    영영 보지 않는다 — M9·M10 은 짝지어진 **그 요소**의 태그·클래스·인라인 선언과 「같은 상태인가」만
#    묻고 **자식의 유무는 묻지 않는다**. 2026-09-26 실측: 목업 `#no-access` 가 규정한 자식 CTA
#    (`btn-request-access`)를 구현이 빠뜨린 채로 게이트가 `rc=0` 이었고, 그 CTA 를 구현에 되돌려
#    주입해도 **11규칙 인쇄가 바이트 동일**이었다(같은 요소의 인라인 `margin-top` 을 흔들면 M9 가
#    좌표까지 지목하므로 규칙이 죽은 것이 아니라 축이 없었던 것이다). **M12 가 그 축을 닫는다** —
#    분모를 단계가 아니라 짝 확정된 요소로 잡아, 활성 단계 승격 없이 자식 카피를 대조한다.
#    닫히지 않은 것은 **짝이 없는 목업 요소**다: 구현이 그 요소를 아예 그리지 않으면 공유 키가
#    생기지 않아 M12 의 분모에도 들어오지 않는다(그쪽은 활성 승격이나 원장 등재로만 닫힌다).
...
def main() -> int:
    ...
    # ── M12 짝 안의 카피 차집합 ──────────────────────────────────────────
    # M3A 의 분모는 「활성 단계」라, 활성이 아닌 단계 안의 상태 블록은 구현이 그것을
    # `data-testid` 로 짝지어 두어도 카피 축에 들어오지 않는다. M9·M10 도 그 요소의
    # 태그·클래스·인라인 선언·「같은 상태인가」만 묻지 **자식의 유무는 묻지 않는다** —
    # 그래서 목업이 규정한 자식 CTA 가 통째로 빠져도 세 규칙이 함께 초록이었다
    # (2026-09-26 `#no-access` 실측: CTA 를 되돌려 주입해도 11규칙 인쇄가 바이트 동일).
    #
    # 여기서는 분모를 단계가 아니라 **짝으로 확정된 요소**로 잡는다. 목업 쪽 요소의 자기
    # 카피를 M3A 와 같은 정규화(`data-sample` 예시값 · `data-variant` 변이 제거)로 추린 뒤,
    # 구현 쪽 짝을 **담은 파일**에 있는지 묻는다. 요소 본문이 아니라 파일인 이유는 구현이
    # 같은 문면을 형제·부모로 옮겨 그릴 수 있기 때문이고(그것은 이탈이 아니다), 그렇다고
    # `frontend/src` 전체로 넓히면 남의 화면 카피가 이 자리의 공백을 가린다.
    paired_missing: list[str] = []
    paired_copy = 0
    file_copy: dict[str, str] = {}
    for key in placed:
        m_file, m_line, _m_tag = mockup_keyed[key]
        i_file, i_line, _i_tag = impl_keyed[key]
        if i_file not in file_copy:
            file_copy[i_file] = " \x01 ".join(impl_copy(SRC_DIR / i_file))
        kept, _taken = split_variants(drop_samples(BODIES[("id", key)]))
        for chunk in own_copy(kept, jsx=False):
            paired_copy += 1
            if chunk in file_copy[i_file] or chunk in exempt_strings:
                note(f"M12 ok [{key}] {chunk}")
                continue
            paired_missing.append(f"`#{key}` 「{chunk}」 — 목업 {m_file}:{m_line} ↔ "
                                  f"구현 {i_file}:{i_line}")
    if not paired_copy:
        fail("M12", f"짝 확정 쌍({len(placed)}쌍)의 목업 자기 카피가 하나도 없다 "
                    f"— 규칙이 공전한다")
    for item in paired_missing:
