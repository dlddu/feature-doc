...
규칙
  ...
  R6 임베드 충실도 — 단계별 임베드 화면 집합 == 터치포인트 줄에서 파싱한 집합,
                     그리고 임베드 본문의 sha256 == 선언된 data-sha256
...
def parse_pages():
    ...
    for p in sorted(MK.glob("*.html")):
        ...
        for m in re.finditer(
                r'<div class="screen-embed[^"]*"\s+data-screen="(S\d\d)"\s+data-source="([^"]+)"\s+data-sha256="([0-9a-f]{64})">',
                s):
            embeds.append({"screen": m.group(1), "source": m.group(2), "sha": m.group(3), "pos": m.start()})
        spans = {}
        for m in re.finditer(r"<!--embed:(S\d\d):begin-->(.*?)<!--embed:\1:end-->", s, re.S):
            spans.setdefault(m.group(1), []).append((m.start(), m.group(2)))
        out[p.name] = {"path": p, "text": s, "journeys": journeys, "sections": sections,
                       "steps": steps, "gotos": gotos, "embeds": embeds, "spans": spans}
    return out
...
for name in journey_files:
    ...
    # R6 임베드 충실도 — 단계별 화면 집합이 터치포인트 파싱 결과와 같은가
    sec_spans = [(m.start(), m.group(1)) for m in re.finditer(r'<section [^>]*\bdata-step="([^"]+)"', pg["text"])]
    sec_spans.append((len(pg["text"]), None))
    by_step = {}
    for i in range(len(sec_spans) - 1):
        start, sid = sec_spans[i]
        end = sec_spans[i + 1][0]
        by_step[sid] = [e["screen"] for e in pg["embeds"] if start < e["pos"] < end]
    for st in jr["steps"]:
        got = sorted(set(by_step.get(st["id"], [])))
        want = sorted(st["screens"])
        if got != want:
            fail("R6", "`%s` 의 `%s` 임베드 화면 %s 이 여정 문서 터치포인트 %s 와 다르다" % (name, st["id"], got, want))

    # R6 임베드 본문 지문
    used = {}
    for e in pg["embeds"]:
        lst = pg["spans"].get(e["screen"], [])
        k = used.get(e["screen"], 0)
        used[e["screen"]] = k + 1
        if k >= len(lst):
            fail("R6", "`%s` 의 %s 임베드에 대응하는 <!--embed:%s:begin/end--> 마커가 없다" % (name, e["screen"], e["screen"]))
            continue
        body = lst[k][1]
        digest = hashlib.sha256(body.encode("utf-8")).hexdigest()
        if digest != e["sha"]:
            fail("R6", "`%s` 의 %s 임베드 본문이 선언된 data-sha256 과 다르다 (선언 %s… / 실측 %s…) — 화면 본문이 임의로 수정됐다"
                 % (name, e["screen"], e["sha"][:12], digest[:12]))
    for sid, lst in sorted(pg["spans"].items()):
        if len(lst) != used.get(sid, 0):
            fail("R6", "`%s` 의 %s 마커 구간 %d개와 screen-embed 선언 %d개가 다르다"
                 % (name, sid, len(lst), used.get(sid, 0)))

    # 남은 원본이 있으면 바이트 동일까지 확인한다 (이관 직후 슬라이스에서 유효)
    for e in pg["embeds"]:
        src = MK / e["source"]
        if src.exists():
            orig = re.search(r"<body[^>]*>(.*)</body>", read(src), re.S)
            if orig and hashlib.sha256(orig.group(1).encode("utf-8")).hexdigest() != e["sha"]:
                fail("R6", "`%s` 의 %s 임베드가 원본 `%s` 의 <body> 와 다르다" % (name, e["screen"], e["source"]))
