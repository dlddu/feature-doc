...
deploy/                    # kind 클러스터 + Deployment + Service (ClusterIP)
├── kind-cluster.yaml
├── deployment.yaml
└── service.yaml
...
스크립트는 kind 클러스터 생성 → docker build → `kind load docker-image` → `kubectl apply` → rollout 대기 → `kubectl port-forward` → smoke.sh + Playwright 실행 → 종료 시 port-forward·클러스터 정리 순서로 한 명령으로 그린까지 갑니다. 클러스터를 남기려면 `KEEP_CLUSTER=1 bash scripts/e2e.sh`.

kind 노드 이미지는 `kindest/node:v1.34.3@sha256:08497ee1…dd48` digest로 핀.
