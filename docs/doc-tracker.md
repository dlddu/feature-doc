...
- e2e spec (`e2e/tests/`): **6개** — AC 전용 1:1 매핑 6건 · 예외 0건 · 공백 18건 (아래 "e2e 매핑")
...
**공백 (18건) — 구현 미착수, 위 "구현 수렴 로드맵"이 닫는다**

전용 spec을 쓸 화면·라우트가 아직 없어 비어 있는 AC. 자매 모델 `tbm_feature-doc-docs-impl`이 추적하는 미구현 18건과 정확히 같은 집합이며, 각 슬라이스가 구현을 얹을 때 그 슬라이스가 spec도 함께 만든다.

| 공백 AC | 닫을 슬라이스 |
|---------|---------------|
| AC1.5 분석 작업의 비동기 실행·진행 가시성 | 3b |
...
- AC: **24** · 예외: **0** · 매핑된 AC(전용 spec 보유): **6** · 공백: **18**
- 매칭 단위 파일: **6** — 전부 정확히 1개 AC 선언(중복 선언 0, 고아 파일 0)
- 항등식 `(AC 24 − 예외 0) = 매핑 파일 6 + 공백 18` ✅
...
| S02 | 저장소 카드의 `23 features · 4 conflicts`, 진행률 62% / `step 3 of 5` | 브랜치 · 최신 분석 상태 배지(`Queued`·`Fetched`…) · 추정 LLM 호출/비용 | 단계별 진행의 화면 표현(AC1.5, 슬라이스 3b) — 단계 데이터 자체는 3a에서 적재된다 |
...
> 알려진 편차: **4건** (모두 2026-08-07 `rct_20260807-0001` 슬라이스 2b에서 등재).
...
미구현 AC 18건(AC1.2~1.5 · AC2.1~2.6 · AC3.1~3.5 · AC4.4 · AC4.6 · AC4.7)을 문서 → 구현으로 수렴시키는 권장 슬라이스 순서. 순서 근거는 **사용자 여정 순서**(연결 → 발견 → 문서)와 **기술 의존성**(분석 파이프라인은 큐·워커 기질이, feature 표현은 분석 산출물이, 문서 관리는 표현이 먼저 필요)이다. AC4.4(모바일 우선)는 각 슬라이스의 화면마다 적용하고, AC4.6(관측·비용)·AC4.7(격리)은 파이프라인 구축 시 내장한 뒤 마지막 슬라이스에서 마감 검증한다. (수립: 2026-07-12 `rct_20260712-0001`; 진행 갱신: 2026-08-02 `rct_20260802-0001`, 2026-08-07 `rct_20260807-0001`, 2026-08-07 `rct_20260807-0002`)

> **진행 현황 (2026-08-07):** 슬라이스 2가 **2a(백엔드 enqueue 계약, PR #8 `dd5e6ee`, `rct_20260715-0001`) + 2b(S02·S03 프론트, PR #11, `rct_20260807-0001`)** 로 모두 완료되어 **AC1.1이 구현 완료**됐다. 이어서 슬라이스 3이 2와 같은 이유로 **3a(AC4.5 워크로드 분리·백엔드/배포) + 3b(AC1.5 S04 진행 화면)** 으로 나뉘었고, **3a가 완료되어 AC4.5가 구현 완료**됐다(`rct_20260807-0002`). 아직 미구현으로 남은 것은 **AC1.2~1.5 · AC2.1~2.6 · AC3.1~3.5 · AC4.4 · AC4.6 · AC4.7**. (다음 권장 슬라이스 = 3b)
...
| 3b | AC1.5 | S04 | 적재된 단계·진행률·비용을 사용자에게 표현하고, 실패 단계의 부분 재시도를 제공. **다음 권장 슬라이스** |
