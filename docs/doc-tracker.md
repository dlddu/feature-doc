...
| AC1.5 비동기 실행·진행 가시성 | `ac1-5-async-progress-and-partial-retry.spec.ts` | 워커 실행 전 S04가 5단계를 `대기 중`으로 표시 → 워커가 `fetch`를 실행하면 `1 of 5`·20%·실측 `766 files · 2.2 MB` → **새로고침 후 동일**(시나리오 5) → S02 카드 `step 1 of 5` → S04 진입 → 실패 단계의 사유 표시와 `이 단계만 다시 시도` → 재시도 후 그 단계만 새 시각으로 재실행되고 나머지 4단계·다른 분석은 불변(시나리오 6) | **재시도가 성공으로 끝나는 왕복** — stub의 실패 원인(없는 브랜치)은 결정적이라 2차 시도도 같은 이유로 실패한다. 재실행 사실은 타임스탬프 갱신으로만 단정한다. 이미 성공한 단계를 보존한 채 실패 단계만 리셋하는 계약은 `backend/tests/progress.rs`가 별도로 지킨다. **실측 누적 비용**은 AC4.6 미구현이라 화면에 없다 |
...
**공백 (17건) — 구현 미착수, 위 "구현 수렴 로드맵"이 닫는다**

전용 spec을 쓸 화면·라우트가 아직 없어 비어 있는 AC. 자매 모델 `tbm_feature-doc-docs-impl`이 추적하는 미구현 17건과 정확히 같은 집합이며, 각 슬라이스가 구현을 얹을 때 그 슬라이스가 spec도 함께 만든다.
...
> 구현 시 예외 등재 후보(모델 정의가 예시로 든 것): ~~AC4.5의 워커 강제 종료·수평 확장~~(3a에서 자동화됨 — 등재하지 않았다), AC4.6의 비용·관측 수치, AC4.7의 격리 검증, AC1.2~AC1.4의 LLM 산출물 품질. 판단은 각 슬라이스의 계획 단계에서 하고, 그 전에는 미리 등재하지 않는다.

**집계**

- AC: **24** · 예외: **0** · 매핑된 AC(전용 spec 보유): **7** · 공백: **17**
- 매칭 단위 파일: **7** — 전부 정확히 1개 AC 선언(중복 선언 0, 고아 파일 0). `e2e/support/`는 최상위가 아니므로 매칭 단위에 들지 않는다
- 항등식 `(AC 24 − 예외 0) = 매핑 파일 7 + 공백 17` ✅
...
**대조 단위는 손으로 정하지 않는다 — 이미 선언돼 있다.** 한 화면이 여러 단계를 그리거나(S01 이
`STP-grant-repo-access` 와 `STP-register-llm-key` 둘을 덮는다) 한 단계가 여러 화면에 걸치는 것
(`STP-pick-target` 이 S02 와 S03 에 걸친다)은 **이탈이 아니라 선언된 구성**이다. 원천은 여정 문서
[`user-journey/JRN-connect-repo.md`](user-journey/JRN-connect-repo.md) 각 단계의 「터치포인트」
줄이고, 목업은 그것을 `data-screens` 속성으로 인용한다([`mockups/README.md`](mockups/README.md)
「여정 페이지 규약」 — **SSOT 이중화 금지**). 아래 표의 「대조 단계」 칸은 그 `data-screens` 의
역인덱스이며, **체커가 파생과 표의 일치를 강제한다**(M1). 표를 손으로 고쳐 선언과 갈라놓을 수 없다.

세 출처가 실제로 일치한다는 것은 기계로 확인된다 — 여정 문서 터치포인트 5줄, 목업 `data-screens`
5개, 컴포넌트 상단 주석의 화면 ID 4개가 모순 없이 같은 매핑을 가리킨다:

| 단계 | 여정 문서 터치포인트 | 목업 `data-screens` | 그 화면의 구현 |
|------|----------------------|---------------------|----------------|
| `STP-sign-in` | (미표현 — 수용된 위험) | (없음) | — |
| `STP-grant-repo-access` | S01 자격증명 등록 | `S01` | `CredentialsSetup.tsx` |
| `STP-register-llm-key` | S01 자격증명 등록 | `S01` | 〃 |
| `STP-pick-target` | S02 저장소 목록 · S03 저장소 연결 | `S02 S03` | `HomeRepositories.tsx` · `ConnectRepository.tsx` |
| `STP-confirm-cost` | S03 저장소 연결 | `S03` | `ConnectRepository.tsx` |

따라서 **선언이 있는 화면은 「대조 보류」에 둘 수 없다** — 무엇과 무엇을 대조할지가 이미 정해져
있으므로 보류할 것이 없다. 보류는 **선언 자체가 없는 화면**, 즉 대응 목업이 아직 여정 페이지로
이관되지 않아 `data-step` 이 없는 화면만의 자리이며, 이것도 **상한 래칫**으로 붙잡는다(면제가
아니다 — 늘면 CI 가 실패한다).
...
| 구현 화면 | 목업 | 대조 단계 |
|-----------|------|-----------|
| `frontend/src/CredentialsSetup.tsx` | `docs/mockups/JRN-connect-repo.html` | `STP-grant-repo-access` · `STP-register-llm-key` |
| `frontend/src/HomeRepositories.tsx` | `docs/mockups/JRN-connect-repo.html` | `STP-pick-target` |
| `frontend/src/ConnectRepository.tsx` | `docs/mockups/JRN-connect-repo.html` | `STP-pick-target` · `STP-confirm-cost` |

### 대조 보류

| 구현 화면 | 목업 | 대조 단계 | 보류 사유(선결) | 해소 시점 |
|-----------|------|-----------|-----------------|-----------|
| `frontend/src/AnalysisProgress.tsx` | `docs/mockups/s04-analysis-progress.html` | (화면 단위 목업 — 단계 없음) | 대응 목업이 아직 여정 페이지로 이관되지 않아 `data-step` 이 없다. 어느 단계와 대조할지가 **선언되지 않았으므로** 대조 단위가 성립하지 않는다. 이관은 `tbm_feature-doc-journey-mockup` 의 몫이다 | `JRN-*` 이관 시 |

> 대조 보류 상한: **1**. 늘면 CI 실패 — 새 화면은 목업이 단계를 선언했으면 **활성으로만** 들어온다.
> 보류로 들어올 수 있는 것은 여정 이관 전의 화면 단위 목업뿐이고, 그때는 상한을 함께 올려 그
> 사실이 diff 에 남게 한다.

## 알려진 목업↔구현 편차

구현이 대응 목업과 어긋나는 지점을 **전부** 여기에 남겨 후속 슬라이스의 입력이 되게 한다. 숨기지 않는
것이 이 표의 목적이고, 등재는 면제가 아니라 부채 기록이다. 유형은 다섯이다.

- **데이터 부재** — 목업이 전제하는 값·API 가 아직 없다. 지어내지 않고 미룬다.
- **제3자 소유** — 목업(여정 프로토타입)이 여정 완결성을 위해 그리지만 실제 화면은 GitHub 등
  외부가 소유한다. 구현은 리다이렉트하며, 구조적으로 수렴하지 않는다.
- **단계·화면 구성** — 선언된 단계↔화면 관계가 1:1 이 아니어서 생기는 차이. 한 화면이 여러 단계를
  덮거나(S01), 한 단계가 여러 화면에 걸치거나(`STP-pick-target`), 단계에 대응 화면이 선언돼 있지
  않은(`STP-sign-in`) 경우다. **결정은 이미 서 있다** — 위 「목업↔구현 대조 범위」 참조.
- **프로토타입 정적성** — 목업은 정적 HTML 이라 로딩·전이·상태별 변형을 그리지 않는다. 구현은
  실제 서버를 기다리므로 그 상태들이 존재한다.
- **미구현 진입점** — 뒷받침하는 API 는 이미 있는데 UI 만 아직 붙지 않았다. 데이터 부재와 달리
  기다릴 것이 없고 붙이면 닫힌다.

각 행의 **백틱 토큰만 기계 대조 대상이다**(나머지 산문은 사람용). 목업 측 토큰은 현재 목업에,
구현 측 토큰은 그 대상 파일에 실재해야 한다 — 삭제된 목업을 근거로 남은 **공전 행**을 이 규칙이
잡는다. `(단계 전체)` 가 붙은 행은 그 단계 전체를 대조에서 뺀다.

| 대상 | 목업이 표현하는 것 | 현재 구현 | 유형 | 사유 | 해소 시점 |
|------|--------------------|-----------|------|------|-----------|
| `docs/mockups/JRN-connect-repo.html` | `STP-grant-repo-access` (단계 전체) | 없음 — `frontend/src/CredentialsSetup.tsx` 가 `Connect GitHub App` 으로 github.com 설치 화면에 리다이렉트한다 | 제3자 소유 | GitHub App 설치 동의(범위 선택·저장소 체크·`Install & Authorize`)는 GitHub 이 소유하는 화면이다. 여정 프로토타입은 흐름이 끊기지 않게 그것까지 그리지만, 제품이 그 화면을 렌더링하면 그것이야말로 사실과 다르다. **구조적 편차이며 해소되지 않는다** | 해소되지 않음(구조적) |
| `frontend/src/CredentialsSetup.tsx` | `여기서 고른 저장소에만 접근합니다. 설치는 GitHub에서 언제든 해제할 수 있어요.` | `읽기 전용 최소 권한만 요청하고, 접근할 저장소는 당신이 직접 고릅니다. LLM 호출은 당신의 키로만 일어나요.`(미인증) · `읽기 전용 최소 권한만 요청합니다. 접근할 저장소는 GitHub에서 직접 고릅니다.`(설치 전) | 제3자 소유 | 위 행의 뒷면이다. 목업은 저장소 선택이 **화면 안에서** 일어난다고 쓰지만 구현은 GitHub 으로 보내므로 「여기서 고른」이 사실이 아니게 된다. 구현이 사실대로 적은 결과 두 문장으로 갈라졌다 | 해소되지 않음(구조적) |
| `docs/mockups/JRN-connect-repo.html` | `STP-sign-in` (단계 전체) | `frontend/src/CredentialsSetup.tsx` 의 미인증 분기(`Sign in with GitHub`) | 단계·화면 구성 | 이 단계는 `data-screens` 가 비어 있어 **대응 화면이 선언돼 있지 않다**. 로그인 상태에 화면 ID(S11 이후)를 부여할지가 선결이며, 「수용된 위험」의 와이어프레임 공백 항목과 같은 뿌리다 | 로그인 화면 ID 부여 시 |
| `docs/mockups/JRN-connect-repo.html` · `frontend/src/CredentialsSetup.tsx` | `분석 비용을 낼 키` · `모든 LLM 호출은 당신의 키로 수행돼요. FeatureDoc은 키를 보관할 뿐 소유하지 않습니다.` · `설치한 App 범위는 그대로 보존돼 있어요. 키 등록만 마치면 이어서 진행됩니다.` | `Bring your own keys` · `모든 LLM 호출과 GitHub 접근은 당신의 자격증명으로 수행돼요. FeatureDoc은 키를 대신 보관할 뿐 소유하지 않습니다.` — 제목·안내가 각각 하나뿐이고, 복귀 안내는 없다 | 단계·화면 구성 | S01 이 `STP-grant-repo-access` 와 `STP-register-llm-key` 를 **함께** 그린다고 여정 문서·목업이 선언했다(`data-screens`). 한 화면에 제목은 하나뿐이므로 두 단계의 제목·안내는 합쳐질 수밖에 없고, 구현의 안내가 GitHub 접근까지 포함하도록 넓어진 것도 그 결과다. 복귀 안내는 두 단계가 한 화면에 있어 **복귀 시점 자체가 없다** | 해소되지 않음(구조적) |
| `docs/mockups/JRN-connect-repo.html` · `frontend/src/CredentialsSetup.tsx` | `저장하고 계속` · `나중에 하기` | `키 등록 및 검증`(키 등록) + 화면 하단 `Continue` · `Repositories →` · `준비 완료 ✓`(다음 화면) — 건너뛰기 액션은 없다 | 단계·화면 구성 | 한 화면이 두 단계를 덮으므로 단계 CTA 하나가 아니라 「키를 등록한다」와 「다음 화면으로 간다」가 각각의 버튼으로 갈라진다. `나중에 하기` 는 구현이 두 자격증명을 **모두** 갖춘 뒤에만 전진시키므로 제공하지 않는다 | 해소되지 않음(구조적) |
| `docs/mockups/JRN-connect-repo.html` · `frontend/src/CredentialsSetup.tsx` | `Bring your own key` — 단계별 앱바(뒤로가기 + 단계 제목) | 상단 브랜드 행 + `Step 1 / 1` | 단계·화면 구성 | 두 단계가 한 화면이라 단계별 앱바가 성립하지 않는다. 온보딩이 한 화면임을 알리는 표시로 대체했다 | 해소되지 않음(구조적) |
| `frontend/src/CredentialsSetup.tsx` | 없음 — 목업은 단계마다 전진 조건이 하나뿐이라 게이팅 안내가 없다 | `자격증명이 준비됐어요. 다음 단계로 이동할 수 있어요.` · `GitHub App 연결과 API Key 등록을 마치면 계속할 수 있어요.` · `GitHub App을 연결하면 계속할 수 있어요.` · `API Key를 등록하면 계속할 수 있어요.` | 단계·화면 구성 | 한 화면이 두 자격증명을 함께 게이트하므로 「무엇이 아직 남았는지」를 조합별로 알려야 한다 — 단계가 나뉘어 있으면 필요 없는 안내다 | 해소되지 않음(구조적) |
| `frontend/src/HomeRepositories.tsx` | `Repositories` — 여정 페이지는 단계 머리말이 맥락을 주므로 앱 브랜드를 그리지 않는다 | 앱바에 브랜드 `FeatureDoc` 을 함께 표시 | 단계·화면 구성 | S02 는 여정의 **재방문 시작점**이라(여정 문서 `STP-pick-target` 터치포인트) 앱을 다시 열었을 때 먼저 닿는 화면이다. 목업은 여정 안에서만 열리므로 그 맥락이 필요 없다 | 해소되지 않음(구조적) |
| `frontend/src/HomeRepositories.tsx` | 없음 — 여정 페이지는 `새 저장소 연결` 단계로 곧장 전진하므로 목록 화면에 진입점 버튼이 없다 | 섹션 액션 `New` | 단계·화면 구성 | 구 `s02-*.html` 에는 진입점이 있었고 2026-08-30 여정 이관과 함께 사라졌다. 프로토타입은 단계 링크로 넘어가지만 제품에는 목록에서 연결로 가는 실제 버튼이 필요하다 | 목업이 목록 화면의 진입점을 다시 그리는 시점 |
| `frontend/src/HomeRepositories.tsx` | 없음 — 여정 페이지에는 분석 목록 구획이 없다 | `진행 상황` 구획 + 카드 안내 `App 설치 범위 밖 — 다시 분석하려면 설치 범위에 추가해 주세요.` | 단계·화면 구성 | `STP-pick-target` 이 S02·S03 둘에 걸치는데 목업은 「고르기」 흐름만 그린다. 진행 중·완료된 분석을 다시 찾아오는 면은 S02 에만 있고 목업에 대응이 없다 | 목업이 목록 화면의 분석 구획을 그리는 시점 |
| `frontend/src/HomeRepositories.tsx` | `아직 분석한 저장소가 없어요. 아래에서 첫 저장소를 연결해 보세요.` | `아직 접근할 수 있는 저장소가 없어요. GitHub App을 연결하고 분석할 저장소를 고르면 여기에 나타납니다.` + `GitHub App 연결하기` | 단계·화면 구성 | 구현의 빈 상태는 「분석 이력 없음」이 아니라 「App 설치 범위에 저장소 없음」을 가리킨다 — 목록 화면이 분리돼 있어 안내와 복구 액션이 달라졌다 | 해소되지 않음(구조적) |
| `frontend/src/HomeRepositories.tsx` | 없음 — 여정 페이지에는 메트릭 그리드가 없다 | 메트릭 그리드 `Repos` · `Analyses` · `Est. cost` | 단계·화면 구성 | 2026-08-30 여정 이관으로 목업에서 메트릭 그리드가 사라졌다(구 `s02-*.html` 의 `Features`·`Spend` 는 파일과 함께 삭제). 목록 화면의 요약 지표를 목업이 다시 정의하면 그때 맞춘다 | 목업이 목록 화면의 요약 지표를 정의하는 시점 |
| `frontend/src/HomeRepositories.tsx` | 없음 — 여정 페이지에는 탭바가 없다 | 하단 탭바 4슬롯(`Repos` 현재 · `Keys` 이동 · `Activity` · `Settings` 비활성) + 비활성 안내 `분석 파이프라인 슬라이스에서 열립니다` · `설정 화면은 후속 슬라이스입니다` | 단계·화면 구성 | 위와 같은 뿌리 — 여정 프로토타입은 한 여정만 그리므로 여정 사이를 오가는 네비게이션이 없다. 비활성 2슬롯은 각 목적지 화면이 생기는 슬라이스에서 열린다 | 각 목적지 화면이 생기는 슬라이스 |
| `frontend/src/ConnectRepository.tsx` | `새 저장소 연결` · `얼마나 들까요` · `분석은 이 버튼으로만 시작돼요. 자동으로 시작되는 경로는 없습니다.` | `Connect a repository` — 한 화면이 두 단계를 덮으므로 제목이 하나뿐이다 | 단계·화면 구성 | 목업은 `STP-pick-target` 과 `STP-confirm-cost` 를 각각 제 화면으로 그리고, 구현은 둘을 한 화면의 두 단계(추정 → 시작)로 합쳤다 — `data-screens` 가 S03 을 두 단계 모두에 붙여 선언한 그대로다 | 해소되지 않음(구조적) |
| `frontend/src/ConnectRepository.tsx` | `Pre-flight` | `New Repository` | 단계·화면 구성 | 위와 같은 뿌리 — 앱바 제목도 하나뿐이다 | 해소되지 않음(구조적) |
| `frontend/src/CredentialsSetup.tsx` · `frontend/src/HomeRepositories.tsx` · `frontend/src/ConnectRepository.tsx` | 없음 — 여정 페이지는 화면 범례 대신 단계 머리말로 맥락을 준다 | `— onboarding · single step` · `— discovery · home view` · `— discovery · connect repository` | 단계·화면 구성 | 구 화면 단위 목업(`s01`~`s03`)의 범례가 구현에 남아 있다. 여정 페이지의 `legend` 는 살아 있지만 쓰임이 다르다(`repositories selected` 같은 상태 보조). 세 화면을 함께 정리해야 해서 개별 수렴 대상이 아니다 | 범례 제거 여부 결정 시 |
| `frontend/src/CredentialsSetup.tsx` · `frontend/src/HomeRepositories.tsx` · `frontend/src/ConnectRepository.tsx` | 없음 — 목업은 정적 프로토타입이라 네트워크 대기 상태를 그리지 않는다 | `불러오는 중…` · `여는 중…` · `검증 중…` · `확인 중…` · `시작하는 중…` | 프로토타입 정적성 | 구현은 실제로 서버를 기다리므로 각 대기 구간에 라벨이 필요하다. 목업에 로딩 상태를 넣으면 그때 대조 대상이 된다 | 목업에 로딩 상태를 넣는 시점 |
| `frontend/src/CredentialsSetup.tsx` · `frontend/src/HomeRepositories.tsx` | 없음 — 목업은 각 화면을 한 상태로만 그린다 | 상태 배지 `Installed` · `Active` · `Invalid`(S01) · `Queued` · `Analyzing` · `Fetched` · `Synced` · `Failed`(S02) | 프로토타입 정적성 | 실제 화면은 서버가 돌려준 상태에 따라 배지가 갈린다. 프로토타입은 대표 상태 하나만 그리므로 나머지 배지에 대응이 없다 | 목업이 상태별 변형을 그리는 시점 |
| `frontend/src/CredentialsSetup.tsx` | 없음 — 목업은 키를 **처음 등록하는** 순간만 그린다 | 이미 등록된 키의 관리 면 — `Remove` | 프로토타입 정적성 | 등록 후 상태(교체·삭제)가 목업에 없다. 구현은 재방문 시 그 상태로 열리므로 액션이 필요하다 | 목업이 등록 후 상태를 그리는 시점 |
| `frontend/src/CredentialsSetup.tsx` | `sk-ant-…` — 목업의 provider select 는 기본값 하나의 placeholder 만 그린다 | provider 별 placeholder `sk-…`(OpenAI) · `AIza…`(Google) | 프로토타입 정적성 | 세 provider 는 목업에도 있으나(`Anthropic` · `OpenAI` · `Google`) 고를 때 바뀌는 placeholder 는 정적 HTML 이 표현하지 않는다 | 목업이 provider 전환을 그리는 시점 |
| `frontend/src/HomeRepositories.tsx` | 없음 — 목업의 저장소 카드는 모두 분석 이력이 있는 상태다 | 미분석 저장소 표시 `· not analyzed` | 프로토타입 정적성 | 구현은 App 이 접근할 수 있는 저장소를 전부 나열하므로 아직 분석하지 않은 것이 섞인다 | 목업이 미분석 카드를 그리는 시점 |
| `frontend/src/ConnectRepository.tsx` | 없음 — 여정 페이지에는 접근 여부 표시 칸이 없다 | `GitHub App` 칸의 `✓ has access` · `✕ no access` · `미확인` | 프로토타입 정적성 | 구현은 pre-flight 를 눌러 확인하는 2단계라 「아직 확인 안 함」 상태가 존재한다. 목업 프로토타입에는 그 중간 상태가 없다 | 해소되지 않음(구조적) |
| `frontend/src/HomeRepositories.tsx` | `로그아웃` | 없음 — 화면에 로그아웃 액션이 없다 | 미구현 진입점 | 백엔드는 `POST /api/auth/logout` 을 이미 갖고 있고(AC4.8 e2e 가 그 경로로 검증한다) **UI 진입점만 없다**. 데이터가 없어서가 아니라 아직 붙이지 않아서 비어 있다 | 앱바 액션을 붙이는 슬라이스 |
| `docs/mockups/JRN-connect-repo.html` · `frontend/src/CredentialsSetup.tsx` | `키 형식이 올바르지 않아요.` · `Anthropic 키는` · `로 시작합니다.` | 없음 — 서버가 돌려준 오류 메시지를 `Invalid` 배지와 함께 그대로 표시한다 | 데이터 부재 | 클라이언트측 키 형식 검증이 없다. 지금 문구만 심으면 검증 없이 문구만 생기므로 `ConnectRepository` 의 URL 형식 검증과 같은 슬라이스에서 함께 넣는다 | 입력 검증을 붙이는 슬라이스 |
| `docs/mockups/JRN-connect-repo.html` · `frontend/src/CredentialsSetup.tsx` | `봉투 암호화로 저장되고 호출 직전에만 복호화됩니다. 폐기하면 진행 중인 분석은 현재 호출까지만 마치고 멈춰요.` | `키는 봉투 암호화(envelope encryption)로 저장되며, LLM 호출 직전에만 메모리에서 복호화됩니다.` — 앞 문장만 있다 | 데이터 부재 | 앞 문장은 같은 사실을 더 자세히 적은 것이라 수렴할 것이 없다. 뒤 문장은 **키 폐기가 진행 중 분석에 어떻게 작용하는지에 대한 사실 주장**인데 그 동작이 아직 정의·검증돼 있지 않다 — 확인 없이 옮기면 거짓 안내가 된다 | 키 폐기 시 분석 동작이 정의·검증되는 시점 |
...
> 알려진 편차: **35건** · 미해소 편차 상한: **35**.
> 2026-08-31 `rct_20260831-0002` 에서 유형 「분할·병합 미결정」을 **폐기**했다 — 화면 구성은
> 미결정이 아니라 여정 문서 「터치포인트」와 목업 `data-screens` 로 이미 선언돼 있었고, 구현이
> 그것과 일치한다. 해당 행들을 실제 사유의 유형으로 재분류하고 「화면 분할 여부 결정 시」라는
> 해소 시점을 전부 실제 시점으로 바꿨으며, S01·S02 를 활성 대조로 승격하면서 드러난 차이를 등재했다.
> 직전 재기준선은 2026-08-31 `rct_20260831-0001`(공전 행 제거 · 유형 도입 · 6칸 기계 판독 형식).
...
| 2026-08-31 「화면 분할 여부」 미결정 해소 | 직전 변경(바로 위)이 남긴 「대조 보류 3건」의 사유는 **사실이 아니었다** — 「구현이 목업 두 단계를 한 화면으로 병합했다/한 단계를 두 화면으로 분할했다, 어느 카피가 어느 화면 몫인지는 제품 결정이다」라고 적었지만, 그 결정은 이미 서 있었고 문서에 있었다. 여정 문서 [`user-journey/JRN-connect-repo.md`](user-journey/JRN-connect-repo.md) 각 단계의 「터치포인트」 줄이 단계→화면을 선언하고(`STP-grant-repo-access`·`STP-register-llm-key` → S01 / `STP-pick-target` → S02+S03 / `STP-confirm-cost` → S03), 목업이 그것을 `data-screens` 로 인용하며, 구현 4화면의 상단 주석 화면 ID 가 그것과 정확히 일치한다 — 세 출처가 모순 없이 같은 매핑을 가리킨다는 것을 기계로 확인했다. 즉 **병합·분할은 이탈이 아니라 선언된 구성**이었고, 원장의 유형 「분할·병합 미결정」은 없는 미결정을 근거로 화면 3개 중 2개의 대조와 원장 다수 행을 붙잡고 있었다. ① 「대조 범위」의 「대조 단계」 칸을 **`data-screens` 의 역인덱스로 파생**하게 하고 체커 M1 이 표와 파생의 일치를 강제한다 — 표를 손으로 고쳐 선언과 갈라놓을 수 없고, **선언이 있는 화면은 보류할 수 없다**(보류는 여정 이관 전이라 `data-step` 이 없는 화면만의 자리다) ② `CredentialsSetup`(S01)·`HomeRepositories`(S02)를 **활성 대조로 승격** — M3B 가 대조하는 구현 카피가 26건 → 87건, M3A 가 대조하는 목업 카피가 36건 → 53건으로 늘었다. `STP-register-llm-key` 의 `(단계 전체)` 면제도 함께 걷었다(그 면제의 근거가 사라진 미결정이었다) ③ 원장 유형 「분할·병합 미결정」을 폐기하고 다섯 유형(데이터 부재 / 제3자 소유 / **단계·화면 구성** / **프로토타입 정적성** / **미구현 진입점**)으로 재분류 — 「화면 분할 여부 결정 시」라는 해소 시점이 0건이 됐다 ④ 승격이 드러낸 차이 전량을 등재(22행 → 35행). **구현은 한 줄도 바꾸지 않았다** — 직전 슬라이스가 무전제 수렴 5건을 이미 처리했고, 남은 차이는 전부 선언된 구성·프로토타입 정적성·데이터 부재에서 오는 것이라 목업을 SSOT 로 삼아도 구현이 움직일 자리가 없었다. 부수: 구현측 카피 추출이 템플릿 리터럴의 `${…}` 표현식을 카피로 세던 오검출 2건 수정, 헤더의 사실과 다른 한계 설명(모듈 상수 라벨 미검출 — 실제로는 검출된다) 정정 (reconciler `rct_20260831-0002`, 모델 `tbm_feature-doc-mockup-render`) | 활성 대조 1 · 대조 보류 3(사유 「제품 결정 대기」) · 원장 22행 · 유형 3종 · 대조 카피 62건 | 활성 대조 3 · 대조 보류 1(사유 「여정 이관 전」) · 원장 35행 · 유형 5종 · 대조 카피 140건 · 범위 표는 `data-screens` 파생 |
...
미구현 AC 17건(AC1.2~1.4 · AC2.1~2.6 · AC3.1~3.5 · AC4.4 · AC4.6 · AC4.7)을 문서 → 구현으로 수렴시키는 권장 슬라이스 순서. 순서 근거는 **사용자 여정 순서**(연결 → 발견 → 문서)와 **기술 의존성**(분석 파이프라인은 큐·워커 기질이, feature 표현은 분석 산출물이, 문서 관리는 표현이 먼저 필요)이다. AC4.4(모바일 우선)는 각 슬라이스의 화면마다 적용하고, AC4.6(관측·비용)·AC4.7(격리)은 파이프라인 구축 시 내장한 뒤 마지막 슬라이스에서 마감 검증한다. (수립: 2026-07-12 `rct_20260712-0001`; 진행 갱신: 2026-08-02 `rct_20260802-0001`, 2026-08-07 `rct_20260807-0001`, 2026-08-07 `rct_20260807-0002`, 2026-08-13 `rct_20260813-0001`)

> **진행 현황 (2026-08-13):** 슬라이스 2가 **2a(백엔드 enqueue 계약, PR #8 `dd5e6ee`, `rct_20260715-0001`) + 2b(S02·S03 프론트, PR #11, `rct_20260807-0001`)** 로 완료되어 **AC1.1이 구현 완료**됐고, 슬라이스 3도 **3a(AC4.5 워크로드 분리, `rct_20260807-0002`) + 3b(AC1.5 S04 진행 화면·부분 재시도, `rct_20260813-0001`)** 로 모두 완료되어 **AC4.5·AC1.5가 구현 완료**됐다. 아직 미구현으로 남은 것은 **AC1.2~1.4 · AC2.1~2.6 · AC3.1~3.5 · AC4.4 · AC4.6 · AC4.7**. (다음 권장 슬라이스 = 4)
...
| 4 | AC1.2 → AC1.3 → AC1.4 | S05 · S06 · S07 | 분석 파이프라인(횡단 관심사 → 탐색 전략 → feature 후보), AC4.6 계측 내장. **다음 권장 슬라이스** |
