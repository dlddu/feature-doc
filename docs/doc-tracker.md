...
## 현재 상태 요약
- **마지막 검증 시점**: 2026-08-27 (design-doc 정합성 검증 + 문서 공개)
- 정의된 가치: **8개** (V1 ~ V8)
...
- 공개된 문서: **18개** (`docs/` 전체, 허브에서 도달 가능 18/18)
...
- 리더의 `CONFIG.flowFromPath` / `flowHref` 는 **이 레포가 아직 화면 단위 목업을 쓰기 때문에**
...
- **화면 단위 목업 10건**: 목업이 여정(플로우) 단위가 아니라 화면 단위(S01~S10)다.
  `design-doc-structure-validator` 는 "여정 하나 = 목업 페이지 하나"를 기대하므로,
  현재 구조는 그 규칙에서 보면 **화면 단위 잔재**에 해당한다.
  - 다만 허브가 화면을 플로우별로 묶어 보여주고 있어, 플로우 흐름을 훑는 것 자체는 가능하다.
  - 전환하면 얻는 것: 한 페이지에서 단계를 눌러 넘기는 **흐름 체험**, 단계 딥링크(`#<step-id>`).
  - 전환 비용: 플로우 4개 각각에 대해 목업 페이지 재구성 + 단계 식별자 부여
    (현재 여정 문서에는 안정적 단계 식별자가 없다 — `S01`은 화면 ID이지 단계 ID가 아니다).
  - **미결정**: 전환 여부는 결정되지 않았다. 스킬 진입점 6(여정 단위 마이그레이션)의 대상이며,
    한 번에 옮기지 않고 플로우 하나씩 옮기고 그때마다 검증하는 방식을 따른다.

- **문서 → 화면 단방향**: 플로우 문서에서 허브의 화면 구획으로 가는 링크는 리더가 제공하지만,
...
- 공개 안 된 문서 (`docs/` 밖 문서): **없음** ✅ — 18개 문서 전부 `docs/` 아래
- 리더 부재: **없음** ✅ — `docs/reader.html` 설치됨
- 끊긴 문서 링크: **없음** ✅ — 허브의 문서 링크 22건 · 목업 링크 10건 전부 실재 파일로 이어짐
- 허브 불일치: **없음** ✅ — `docs/` 아래 md 18개 전부 허브에서 도달 가능
...
5. **목업의 여정 단위 전환 여부 결정** — 현재 화면 단위(S01~S10). 전환하면 한 페이지에서 단계를 눌러 넘기는 흐름 체험과 단계 딥링크를 얻지만, 플로우 문서에 안정적 단계 식별자를 먼저 부여해야 한다(`S01`은 화면 ID이지 단계 ID가 아님). 전환을 택하면 플로우 하나씩 옮기고 그때마다 검증한다.
6. **목업 → 플로우 문서 복귀 링크** — 목업 10개에 그 화면이 속한 플로우 문서로 돌아가는 링크 추가. 지금은 문서 → 화면 단방향이라, 목업을 먼저 연 사람이 근거 문서로 갈 수 없다.
7. **S11 변경 이력 화면 제작** — 현재 "수용된 위험"으로 보류 중. 제작 시 와이어프레임(`gen-wireframes.js` S11 entry)→목업 순서로 만들고 본 추적 문서의 수용된 위험에서 해제.
