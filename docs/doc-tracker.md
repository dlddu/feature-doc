...
| S02 | 저장소 카드의 `23 features · 4 conflicts`, 진행률 62% / `step 3 of 5` | 브랜치 · 최신 분석 상태 배지 · 추정 LLM 호출/비용 | 워커·단계별 진행(AC1.5 · AC4.5, 슬라이스 3) |
...
미구현 AC 19건(AC1.2~1.5 · AC2.1~2.6 · AC3.1~3.5 · AC4.4~4.7)을 문서 → 구현으로 수렴시키는 권장 슬라이스 순서. 순서 근거는 **사용자 여정 순서**(연결 → 발견 → 문서)와 **기술 의존성**(분석 파이프라인은 큐·워커 기질이, feature 표현은 분석 산출물이, 문서 관리는 표현이 먼저 필요)이다. AC4.4(모바일 우선)는 각 슬라이스의 화면마다 적용하고, AC4.6(관측·비용)·AC4.7(격리)은 파이프라인 구축 시 내장한 뒤 마지막 슬라이스에서 마감 검증한다. (수립: 2026-07-12 `rct_20260712-0001`; 진행 갱신: 2026-08-02 `rct_20260802-0001`, 2026-08-07 `rct_20260807-0001`)

> **진행 현황 (2026-08-07):** 슬라이스 2가 **2a(백엔드 enqueue 계약, PR #8 `dd5e6ee`, `rct_20260715-0001`) + 2b(S02·S03 프론트, PR #11, `rct_20260807-0001`)** 로 모두 완료되어 **AC1.1이 구현 완료**됐다. 아직 미구현으로 남은 것은 **AC1.2~1.5 · AC2.1~2.6 · AC3.1~3.5 · AC4.4~4.7**. (다음 권장 슬라이스 = 3)
...
| 3 | AC4.5 · AC1.5 | S04 | 큐 + API/워커 워크로드 분리, 비동기 진행 가시성. **다음 권장 슬라이스** |
