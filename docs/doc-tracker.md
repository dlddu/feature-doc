...
## 현재 상태 요약
- **마지막 검증 시점**: 2026-09-02 (여정 단위 목업 이관 슬라이스 5 — `JRN-understand-feature` · S08~S10 원본 흡수·삭제. **이관 완료**)
- 정의된 가치: **8개** (V1 ~ V8)
...
- e2e spec (`e2e/tests/`): **9개** — AC 전용 1:1 매핑 9건 · 예외 0건 · 공백 15건 (아래 "e2e 매핑")
...
**공백 (15건) — 구현 미착수, 위 "구현 수렴 로드맵"이 닫는다**

전용 spec을 쓸 화면·라우트가 아직 없어 비어 있는 AC. 자매 모델 `tbm_feature-doc-docs-impl`이 추적하는 미구현 15건과 정확히 같은 집합이며, 각 슬라이스가 구현을 얹을 때 그 슬라이스가 spec도 함께 만든다.

| 공백 AC | 닫을 슬라이스 |
|---------|---------------|
| AC1.4 feature 후보 | 4b-2 |
| AC4.6 관측·비용 | 7 |
| AC2.1~AC2.6 feature 표현·의존성 | 5 |
| AC3.1~AC3.5 문서 CRUD·이력·충돌 | 6 |
| AC4.4 모바일 우선 · AC4.7 분석 작업 격리 | 7 |

> 구현 시 예외 등재 후보(모델 정의가 예시로 든 것): ~~AC4.5의 워커 강제 종료·수평 확장~~(3a에서 자동화됨 — 등재하지 않았다), AC4.6의 비용·관측 수치, AC4.7의 격리 검증, ~~AC1.2~~(4a에서 구조·근거·결정성을 자동 검증했다 — 남는 것은 의미적 품질 한 갈래뿐이라 AC 전체를 면제하는 것은 과잉이고, 등재하지 않았다)·~~AC1.3~~(4b-1에서 구조·근거·편집·승인을 자동 검증했다 — 남는 것은 제안의 의미적 품질뿐이라 등재하지 않았다)·AC1.4의 LLM 산출물 품질. 판단은 각 슬라이스의 계획 단계에서 하고, 그 전에는 미리 등재하지 않는다.

**집계**

- AC: **24** · 예외: **0** · 매핑된 AC(전용 spec 보유): **9** · 공백: **15**
- 매칭 단위 파일: **9** — 전부 정확히 1개 AC 선언(중복 선언 0, 고아 파일 0). `e2e/support/`는 최상위가 아니므로 매칭 단위에 들지 않는다
- 항등식 `(AC 24 − 예외 0) = 매핑 파일 9 + 공백 15` ✅
...
> 슬라이스 4b 는 **4b-1(AC1.3 탐색 전략 생성·검토·수정·승인 + S06, `rct_20260831-0004`)** 로 절반이 끝나
> **AC1.3 이 구현 완료**됐다.
> 아직 미구현으로 남은 것은 **AC1.4 · AC2.1~2.6 · AC3.1~3.5 · AC4.4 · AC4.6 · AC4.7**. (다음 권장 슬라이스 = 4b-2)
...
| 4b-2 | AC1.4 | S07 | 승인된 전략을 따라 코드베이스를 스캔해 feature 후보를 추출하고, 승인/거부와 거부 사유 기록을 붙인다. **다음 권장 슬라이스** |
