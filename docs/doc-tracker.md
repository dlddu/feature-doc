...
## 현재 상태 요약
- **마지막 검증 시점**: 2026-09-01 (여정 단위 목업 이관 슬라이스 4 — `JRN-follow-code-change` · S04·S07 원본 흡수·삭제)
- 정의된 가치: **8개** (V1 ~ V8)
...
10. **AC4.2 에 지원 제공자 범위 조항 추가** — 아래 「LLM 프로바이더 지원 범위」가 스스로 적어 둔 미해소
    공백이다. 구현에는 사용자 대면 동작이 이미 있는데(Google 키는 등록되지만 분석 단계가 "아직 지원하지
    않는다"로 실패한다) **그 동작을 규정하는 AC 가 없다**. 이 표는 메타 상태 파일의 기록이지 SSOT 가
    아니므로, 해소하려면 PRD-4 를 고쳐야 한다. 결정이 필요한 것은 하나다 — *등록은 허용하되 분석에서
    실패하는 지금 동작이 의도인가, 아니면 등록 자체를 막아야 하는가*. 앞쪽이면 AC4.2 에 조항을 더하는
    문서 작업이고, 뒤쪽이면 등록 화면·API 변경이 따른다. **stub 모드에서는 미지원 제공자가
    `DEFAULT_PROVIDER` 로 폴백하므로 e2e 로는 관측되지 않는다** — 검증 수단까지 함께 정해야 한다
    (reconciler `rct_20260831-0004` 가 판정 기준 3 위반으로 등재했고, 슬라이스 4b-1 의 범위 밖으로 남겼다).
