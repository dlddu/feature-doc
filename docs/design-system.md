...
**폭 판정은 기기가 아니라 앱 창 폭**으로 합니다. 구현은 앱 루트를 `container: app / inline-size` 로 두고 `@container app (…)` 로 분기하고, 목업은 `.frame` 이 그 앱 루트 역할을 합니다. 같은 규칙이 목업·구현 양쪽에 그대로 옮겨지게 하기 위해서예요.
...
12개 primitive (4.1 ~ 4.12). 모두 토큰의 조합으로 만들어졌고, **변형 (variants)은 색이 아니라 채워짐·채움 부재로 구분**됩니다.
...
### 4.10 Segment selector

배타적 선택지(예: LLM provider) 한 줄. 활성 항목은 **색이 아니라 채워짐**으로 구분 — Tag·Tabs와 같은 원칙.

```html
<div class="segment">
  <span class="seg active">Anthropic</span>
  <span class="seg">OpenAI</span>
  <span class="seg">Google</span>
</div>
```

- 컨테이너: `display:flex` · 6px gap
- `.seg`: `flex:1` · 가운데 정렬 · 8/4 padding · 1px `--border-default`
- `.seg.active`: `--bg-input` fill · `--border-strong` · `--text-primary` · weight 500

### 4.11 Metric grid

숫자 지표 2~3개를 한 묶음으로 보여주는 분할 카드. 셀은 1px divider로만 나뉘고 강조 색은 없음.

```html
<div class="metric-grid">
  <div class="cell"><span class="k">REPOS</span><span class="v">12</span></div>
  <div class="cell"><span class="k">ANALYZING</span><span class="v">3</span></div>
</div>
```

- 컨테이너: `display:flex` · 1px `--border-default` · 14px radius · `overflow:hidden`
- `.cell`: `flex:1` · 13/14 padding · 셀 사이 1px `--border-default` divider
- `.k`: 9px mono · UPPER · tracking .12em · `--text-tertiary` (Caps label과 동일 역할)
- `.v`: 18px mono · weight 500 · `--text-primary` (Metric 스케일)

### 4.12 Progress bar

선형 진행률. 파이프라인 단계 표현인 §4.5 Step과 달리, **연속적인 비율 하나**를 나타낼 때 사용.

```html
<div class="progress"><i style="width:62%"></i></div>
```

- 트랙: 4px height · 2px radius · `--bg-input`
- fill(`> i`): `--text-secondary` · 2px radius (status 색·accent 사용 금지)
...
§3.4 를 옮긴 기준 CSS. 셀렉터는 목업 기준이고(`.frame` = 앱 루트), 구현에서는 앱 루트 요소에 `container` 를 답니다.
