...
**합계**: 판정 **151파일**(지문의 파일 집합 기준으로는 146 — `format.ts` · `backend/tests/crypto.rs` ·
...
```
행 열의 합 2,807  +  ⑴ 행 열 밖 잔여 0  +  ⑵ 행 열 안 미판정 증분 0  =  전역 live 2,807
```

이 세 값은 **36차 패스의 트리**(PR head, 부모 `origin/main@bace207`)에서 잰 것이다. 36차 패스가
