...
0행이 되어 빠졌다. **파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **2,458행**
(직전 2,375 + 21차 패스(#139) 6 + 22차 패스 16 + 23차 패스(#143) 60 + 24차 패스(#145) 1 — 23·24차는
합계 서술을 갱신하지 않고 넘겼으므로 22차 패스가 이어 적는다) · 판정 범위의 현재 합계 **2,466행**(행 열의 합).
...
행 열의 합은 **2,466 = 판정 완료 2,433 + 미판정 증분 33**.
