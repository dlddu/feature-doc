...
0행이 되어 빠졌다. **파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **2,391행**
(직전 2,375 + 21차 패스 16) · 판정 범위의 현재 합계 **2,458행**(행 열의 합).
**그중 판정 완료는 2,419행**이고 **미판정 증분은 39행**이다 — #114 가 판정 완료 범위 안 네 행(원장 1·4·7·10행)에
...
밖이라 **잔여**로 들어갔다. 행 열의 합은 **2,458 = 판정 완료 2,419 + 미판정 증분 39**.

**21차 패스(이 패스)가 이 트리를 만들었다.** 19차 패스(#128)가 「벽은 걷혔지만 **판정 자체가 없다** …
...
**19차 패스(#128, `c1b39ad`)가 그보다 앞선 트리를 만들었다.** 18차 패스(#124)가 「분류는 다시 재지 않았다」로 넘긴 몫을
...
**직전까지 ⑶ 에 함께 있던 무인 자유 풀 4파일 33행은 21차 패스가 새 행 23 으로 닫았다**
...
- **나머지 4파일 33행에는 벽이 없다 — 무인 자유 풀이다**: `backend/src/db.rs` 17 ·
  ...
  **— 해소됨(2026-09-22 · 21차 패스 · `rct_20260922-0008` · 새 행 23).** 그 4파일 33행은 판정돼 17행이
...
가짜 벽 뒤 `crypto.rs` 13행은 19차 패스(#128)가, 그 벽 뒤에 남아 있던 **4파일 33행은 21차 패스(이 패스)가**
