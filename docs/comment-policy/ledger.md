...
16차 패스 16 — 마이그레이션 축은 전부 새 범위이고 증분 재판정은 없다) · 판정 범위의 현재 합계 **2,197행**
(직전 2,074 + 16차 패스 잔여 123 = 행 열의 합). **판정 범위 안에 남은 미판정 증분은 없다** — 16차 패스의
부모 `7724b46` 이후 main 에 들어온 #93 9행 · #91 5행 · #99 25행 · #101 2행은 네 증분 재판정이 이미 닫았고
(#100 · #102 · #103 · #104), 그 증분들은 마이그레이션 8파일과 겹치지 않는다. 7행의 줄 수·지문은
133/`4aa7a8eb…`, 10행은 81/`af9fbb9d…` 이다.

**미판정 잔여**: **6파일 / 104행**
(16차 패스를 main 위에 병합한 트리 기준 — 전역 `lines=2301 files=108`). 전역 지문의 파일 목록에서 판정
106파일을 **집합으로 뺀** 값이며, 뺄셈과도 일치한다(104 == 2,301 − 2,197).
...
1행 → 2행으로 늘려(+1) D2 몫이 44 → 45행이 됐다(부모 `7724b46` 2,310 → PR head 2,295 실측).

**잔여는 이제 두 몫뿐이고, 둘 다 판정기의 경로 규칙에 걸린다**(4 + 2 = 6파일 · 45 + 59 = 104행).
**무인 자유 풀은 여전히 0이다** — 15차가 자유 풀 5파일 38행을, 16차가 마이그레이션 축 8파일 139행을
가져갔다. 마이그레이션 몫은 이제 **판정된 행**이다(위 표 마지막 행). 이 조건은 PR 번호에 묶이지 않는다 —
**자매 모델의 수렴 슬라이스가 e2e·프런트를 다시 쓸 때마다** 판정 완료 범위 안에 증분이 생기고,
#91·#99 가 그 형태였다.

- **D2 「저장 계층 핵심」 4파일 / 45행 — `backend/src/db.rs` 17 · `backend/src/crypto.rs` 13 ·
  `backend/tests/migrations.rs` 14 · `backend/src/models.rs` 1** (다섯 번째인
...
**다음 무인 패스의 후보 풀은 비어 있다**(미판정 증분 0 — 위 합계). 다음 후보는 자매 슬라이스가 판정 완료
범위 안에 새 주석을 더할 때 그 증분으로만 열린다. 열린 PR #92 는 `backend/src/doc_edit.rs` ·
`backend/tests/doc_edit.rs` 등 **새 파일**을 들여놓으므로 머지되면 잔여(행 없는 파일)도 다시 늘어난다.
**「자유 풀」의 수치는 이 트리에서만 참인 조건부 진술이다** — 다음 감지는 절대 수치를 믿지 말고 **전역
지문의 파일 목록에서 위 106파일을 집합으로 빼서** 잔여를, 행 열의 합과 「전역 − 잔여」의 차로 미판정
증분을 다시 계산해야 한다. **마이그레이션 축은 16차 패스가 가져갔으므로**, 증분 밖에서 남는 것은
D2·D6·D5 경로 벽 6파일 104행뿐이고 **그 벽은 주석 패스가 스스로 풀 수 없다**(판정기 자신을 고치는 것도
D6 다).

**⚠️ 열린 PR #92 가 `backend/migrations/0009_feature_doc_edits.sql` 을 들여온다 — 머지 전이 마지막
무상 기회다.** 그 파일의 머리 주석 28행은 `AC3.1`·`AC3.4`·`AC1.2`·`AC2.6`·`PRD-3` 과
`backend/src/doc_edit.rs` 를 인용하는데, 그것은 `backend/migrations/README.md` 가 **이 디렉터리에
금지한** 바로 그 참조다. **아직 운영에 적용되지 않았으므로 지금 고치면 repair 가 0이다** — 머지돼
롤아웃되는 순간 같은 정리가 16차 패스와 똑같은 사람 게이트(suspend · scale 0 · UPDATE · resume)를
요구하게 된다.
