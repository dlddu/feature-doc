...
0행이 되어 빠졌다. **파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **2,372행**
(직전 2,366 + 19차 패스 6) · 판정 범위의 현재 합계 **2,405행**(전역 `lines=2597` − 잔여 192).
**그중 판정 완료는 2,401행**이고 **미판정 증분은 4행**이다 — #114 가 판정 완료 범위 안 네 행(원장 1·4·7·10행)에
...
재실측」 — 판정하지 않고 다음 감지에 넘긴다). 행 열의 합은 **2,405 = 판정 완료 2,401 + 미판정 증분 4**.

**19차 패스(이 패스)가 이 트리를 만들었다.** 18차 패스(#124)가 「분류는 다시 재지 않았다」로 넘긴 몫을
...
(순 제거 6). **이 패스만의 효과**는 `lines=2603 files=134` → `lines=2597 files=134` 다. 원장 정정은 줄 수를
바꾸지 않는다 — 이번 어긋남은 **분류**였고, 그래서 #124 가 합을 82 로 고친 뒤에는 **산술이 전부 초록인 채로
틀려 있었다**(「읽는 법」이 스스로 적은 사각지대의 실례).

**18차 패스(#124)가 그 직전 트리를 만들었다.** 사람 PR **#114**(`aacd0b4`, 슬라이스 6d — AC3.5 코드 자동 분석과
...
**미판정 잔여**: **10파일 / 192행**
(이 패스의 트리 기준 — 전역 `lines=2597 files=134`). 전역 지문의 파일 목록에서 판정 129파일을 **집합으로 뺀**
값이며, 뺄셈과도 일치한다(192 == 2,597 − 2,405). #114 가 들여온 마이그레이션 `0013_feature_doc_conflicts.sql`
...
- **나머지 4파일 33행에는 벽이 없다 — 무인 자유 풀이다**: `backend/src/db.rs` 17 ·
  ...
  - **무효화된 기록 — 이 두 항목은 2026-09-22 까지 「D2 4파일 45행 + D5 `pvc.yaml` 1행도 벽」이라고 적었다.**
    ...
    **전제가 사라진 것**이다: **#125(`78183d2`, 2026-09-22)가 판정기에서 D2(`CORE_FILES`) ·
    ...
    19차 패스(이 패스)가 그 몫을 받아 같은 방법으로 다시 쟀다.
  - **음성 대조 재실측**(부모 트리 `0fc6552` = #124 착지 직후 main · probe = 그 위에 **주석 한 줄씩만** 덧붙인 트리):
    ...
    **이 패스 자신이 세 번째 증거다** — 가짜 벽 뒤에 있던 `crypto.rs` 를 실제로 편집한 이 PR 이 `✅` 를 받는다.
...
**다음 무인 패스의 후보 풀은 4파일 33행 + 미판정 증분 4행이다.** #114 의 무인 115행은 18차 패스(#124)가
닫았고, 19차 패스(이 패스)가 `crypto.rs` 13행을 닫으면서 **가짜 벽 뒤에 있던 나머지 4파일 33행**
(`db.rs` 17 · `backend/tests/migrations.rs` 14 · `models.rs` 1 · `pvc.yaml` 1)이 자유 풀로 들어왔다.
증분 4행은 #123 이 판정 완료 범위 안에 연 몫이다. 여기에 더해 자매 슬라이스가 판정 완료 범위 안에 새 주석을
더하거나 새 파일을 들여올 때 또 열린다. **「자유 풀」의 수치는 이 트리에서만 참인 조건부 진술이다** —
다음 감지는 절대 수치를 믿지 말고 **전역 지문의 파일 목록에서 위 129파일을 집합으로 빼서** 잔여를,
행 열의 합과 「전역 − 잔여」의 차로 미판정 증분을 다시 계산해야 한다.
