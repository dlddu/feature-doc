...
**파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **1,917행**(직전 1,911 +
증분 재판정 6 — 새 범위 없음, #93 의 9행 중 제거 6 · 유지 3) · 판정 범위의 현재 합계 **2,101행**
(전역 `lines=2343` − 잔여 242). **그중 판정 완료는 2,071행**(직전 2,068 + #93 유입 9 − 제거 6)이고
**미판정 증분 30행**이 판정 완료 범위 안에 들어와 있다 — 이 재판정의 창(#93) 밖에서 온 유입 두 건:
**#91(`3567755`)이 10행 `frontend/src/index.css` 에 더한 5행**(`rct_20260921-0002` 소관 — 그 task 가
열려 있다) · **#99(`899800e`)가 3행 `sc01-01` 에 더한 14행과 8행 `sc01-04` 에 더한 11행**(감지 대기).
행 열의 합은 2,082 — 3행(+14)·10행(+5)의 줄 수·지문은 그 증분을 판정하는 패스가 갱신한다.

**미판정 잔여**: **14파일 / 242행**
(이 재판정 병합 후 트리 기준 — 전역 `lines=2343 files=108`). 전역 지문의 파일 목록에서 판정 98파일을
**집합으로 뺀** 값이며, 뺄셈과도 일치한다(242 == 2,343 − 2,101). #93·#91·#99 는 새 파일을 들여오지
않았으므로 이 잔여는 15차 패스 이후 한 줄도 움직이지 않았다.

**잔여(행 없는 파일)는 세 몫뿐이고, 셋 다 사람 게이트다**(8 + 4 + 2 = 14파일 · 139 + 44 + 59 = 242행).
**무인 자유 풀은 판정 완료 범위 안의 미판정 증분 30행뿐이다**(위 합계 — 15차 패스가 「#92·#93 이
머지되면 다시 열린다」고 적어 둔 조건이 #93 으로 발효됐고, 그 9행은 이 재판정이 닫았다).
...
**다음 무인 패스의 후보 풀은 위 미판정 증분 30행이다**(#91 5행 → `rct_20260921-0002` · #99 25행 →
감지 대기). 열린 PR #92 는 `backend/src/doc_edit.rs` · `backend/tests/doc_edit.rs` 등 **새 파일**을
