...
0행이 되어 빠졌다. **파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **2,479행**
(직전 2,375 + 21차 패스(#139) 6 + 22차 패스 16 + 23차 패스(#143) 60 + 24차 패스(#145) 1 + 25차 패스 21 — 23·24차는
합계 서술을 갱신하지 않고 넘겼으므로 22차 패스가 이어 적었고, 이 줄은 25차 패스가 이어 적는다) ·
판정 범위의 현재 합계 **2,476행**(행 열의 합).
...
행 열의 합은 **2,466 = 판정 완료 2,433 + 미판정 증분 33** 이었다 — **25차 패스가 그 33행을 전부 닫았다**
(행 12 의 증분 14행은 증분 재판정 ⑫ 로, 행 밖 잔여였던 `e2e/support/github-app.ts` 4행과
`e2e/tests/sc01-08-succeeded-stage-rerun.spec.ts` 13행은 **새 행 24** 로). 이 트리의 행 열의 합은
**2,476 = 판정 완료 2,476 + 미판정 증분 0** 이고, 이 패스의 base(`c377c30`)에서 **23행 전부가 줄 수·지문
둘 다 재현된다**(실측 23/23). 이 23/23 은 **그 base 에 묶인 값**이다 — 열린 자매 PR 이 착지하면
그쪽이 연 증분만큼 다시 어긋나고(현재 열린 것: **#146** → 행 9 `CrossCuttingConcerns.tsx` ·
**#126** → 행 4·7·10·21 과 새 파일 5개), 그것은 이 패스의 오류가 아니라 **다음 감지가 여는 몫**이다.

**25차 패스가 이 트리를 만들었다.** 22차 패스가 「다음 감지가 여는 몫」으로 명시 인계한 **행 12 의
