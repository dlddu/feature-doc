...
**합계**: 판정 **148파일**(지문의 파일 집합 기준으로는 143 — `format.ts` · `backend/tests/crypto.rs` ·
...
넘겼으므로 22차 패스가 이어 적었고, 이 줄은 29차 패스가 이어 적는다) ·
판정 범위의 현재 합계 **2,703행**(행 열의 합 — 29차 패스로 **행 열 밖 잔여가 0** 이 됐다).

**산술은 두 축으로 갈라 화해한다** — 한 덩어리로 쓰면 「잔여」가 ⑴·⑵ 를 섞어 버린다.

```
행 열의 합 2,703  +  ⑴ 행 열 밖 잔여 0  +  ⑵ 행 열 안 미판정 증분 7  =  전역 live 2,710
```
...
9 → 12 = 행 17). **29차 패스는 ⑵ 를 판정하지 않는다** — 증분 재판정은 원래 행·원래 패스 파일에
절을 더하는 별도 슬라이스이고(본문 「판정 절차」 3), 열린 #148 이 `e2e` 12파일을 건드리므로 행 3 의
증분은 착지 전에는 확정되지도 않는다. 세 행의 줄 수 칸은 **그 재판정이 옮긴다.**
