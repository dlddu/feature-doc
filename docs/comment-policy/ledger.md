...
**합계**: 판정 **128파일**(지문의 파일 집합 기준으로는 123 — `format.ts` · `backend/tests/crypto.rs` ·
`backend/tests/analyses.rs` · `deploy/k8s/kustomization.yaml` · `backend/tests/settings.rs` 가 전건 제거로 주석
0행이 되어 빠졌다. **파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **2,366행**
(직전 2,300 + 18차 패스 66) · 판정 범위의 현재 합계 **2,398행**(전역 `lines=2603` − 잔여 205).
**그중 판정 완료는 2,394행**이고 **미판정 증분은 4행**이다 — #114 가 판정 완료 범위 안 네 행(원장 1·4·7·10행)에
연 **순 +24행**을 18차 패스가 증분 재판정으로 닫았고(순 제거 11행), 같은 슬라이스가 들여온 새 파일 4개는
**새 행 21**(31행)이 됐다. 남은 4행은 **머지 직전에 착지한 #123** 이 원장 1행에 연 것이다(행 1 「자매 착지
재실측」 — 판정하지 않고 다음 감지에 넘긴다). 행 열의 합은 **2,398 = 판정 완료 2,394 + 미판정 증분 4**.

**18차 패스(이 패스)가 이 트리를 만들었다.** 사람 PR **#114**(`aacd0b4`, 슬라이스 6d — AC3.5 코드 자동 분석과
...
2줄을 줄 수 그대로 고쳐 썼다). 이 트리의 검산은 **21행 전건 재현**과 **행 열의 합 2,398 + 잔여 205 = 2,603** 이다.
...
**미판정 잔여**: **11파일 / 205행**
(머지 시점 트리 기준 — 전역 `lines=2603 files=134`). 전역 지문의 파일 목록에서 판정 128파일을 **집합으로 뺀**
값이며, 뺄셈과도 일치한다(205 == 2,603 − 2,398). #114 가 들여온 마이그레이션 `0013_feature_doc_conflicts.sql`
24행이 잔여에 새로 들어왔다 — 문서 편집 축이 「자매 슬라이스가 새 파일을 들여올 때마다 새 잔여가 생긴다」고 적어
둔 그대로이고(#92 → 0009, #107 → 0010, #112 → 0011, #108 → 0012, **#114 → 0013**), 남은 몫은 전부 사람 게이트다.

**잔여(행 없는 파일)는 일곱 몫뿐이고, 일곱 다 사람 게이트다**(1 + 1 + 1 + 1 + 1 + 4 + 2 = 11파일 · 30 + 30 + 25 + 14 + 24 + 45 + 37 = 205행).
**무인 자유 풀에 남은 것은 #123 이 연 4행뿐이다 — 판정 완료 범위 안의 미판정 증분 4행**(위 합계). 이 조건은 PR 번호에 묶이지
...
- **D2 「저장 계층 핵심」 4파일 / 45행 — `backend/src/db.rs` 17 · `backend/src/crypto.rs` 13 ·
  ...
  **판정까지 마쳐 뒀다**(제거 6 · 유지 7 —
  ...
  「보류분의 판정 결과」).
- **D6 · D5 경로 규칙 2파일 / 37행 — `tools/check-data-format-change.py` 36 ·
  ...
  ⚠️ 머지 직전에 착지한 **#125(`78183d2`)가 이 게이트를 D1·D6 만으로 축소**하며 판정기 자신의 주석을 58 → 36행으로
  줄였고, **#127(`df5afd5`)이 그 status 이름을 `review/data-format` → `review/manual-approval` 로 개명**했다
  (줄 수는 그대로, 주석 2줄 제자리 수정). 줄 수는 위에 재실측해 넣었지만 **D5·D2 벽의 분류 자체는 다시 재지 않았다** — 새 판정기로 음성 대조를
  다시 돌리는 것은 다음 감지·계획의 몫이다.

**다음 무인 패스의 후보 풀은 #123 이 연 4행뿐이다**(#114 의 무인 115행은 18차 패스(이 패스)가 닫았다 — 새 무인
파일 0 · 미판정 증분은 머지 직전 자매 착지분 4행). 다음 후보는 자매 슬라이스가 판정 완료 범위 안에 새 주석을 더하거나 새 파일을 들여올 때 그
증분으로만 열린다. **「자유 풀」의 수치는 이 트리에서만 참인 조건부 진술이다** — 다음 감지는 절대 수치를 믿지
말고 **전역 지문의 파일 목록에서 위 128파일을 집합으로 빼서** 잔여를, 행 열의 합과 「전역 − 잔여」의 차로
미판정 증분을 다시 계산해야 한다.
...
들어올 때 함께 집는다. D2·D6·D5 경로 벽 6파일 82행은 **주석 패스가 스스로 풀 수 없다**(판정기 자신을
고치는 것도 D6 다).
