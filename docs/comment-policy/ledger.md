...
**합계**: 판정 **117파일**(지문의 파일 집합 기준으로는 113 — `format.ts` · `backend/tests/crypto.rs` ·
`backend/tests/analyses.rs` · `deploy/k8s/kustomization.yaml` 이 전건 제거로 주석 0행이 되어 빠졌다.
**파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **2,182행**(직전 2,062 + 이 패스
120 — 새 범위 5파일 106 + 증분 재판정 14) · 판정 범위의 현재 합계 **2,302행**(전역 `lines=2466` − 잔여 164).
**그중 판정 완료는 2,302행**(직전 2,255 + #107 이 판정 행 안에 더한 16 − 제거 14 + 빠진 feature 직접 추가 축
새 행 45)이고 **미판정 증분은 0행**이다 — **#107(`89a1625`, 자매 `tbm_feature-doc-docs-impl` 슬라이스 6b)이
판정 완료 범위 안에 더한 16행**(1행 `analysis.rs` 2 · 2행 `config.rs` 3 · 5행 `deploy/e2e/kustomization.yaml` 2 ·
8행 `FeatureCandidates.tsx` 1 · 10행 `api.ts` 7 · 17행 `doc_edit.rs` 1)을 이 패스의 여섯 증분 재판정(1행 ⑦ · 2행 ③ ·
5행 ③ · 8행 ③ · 10행 ④ · 17행 ①, 제거 14 · 유지 2)이 닫았고, 1·2·5·8·17행의 줄 수·지문은 #107 이전 값으로
**바이트 동일하게 되돌아왔다**(590/`9757b6b5…` · 140/`ddacce0b…` · 190/`632b0475…` · 141/`a431e905…` ·
56/`b9e6778d…`), 10행은 유지 2행만큼 늘어 83/`ec54e678…` → 90/`d88323c6…`(트리거) → **85/`89959a07…`** 다.
같은 커밋이 들여온 **새 파일 6개 / 181행** 중 사람 게이트 없는 5파일 / 151행은 새 행(빠진 feature 직접 추가 축,
제거 106 · 유지 45)으로 닫았고, `0010_feature_additions.sql` 30행은 마이그레이션 축의 잔여로 들어갔다. 행 열의
합은 **2,302 = 판정 완료 2,302** — 판정 완료 범위 안에 미판정 증분이 없다(#93 9행 · #91 5행 · #99 25행 ·
#101 2행 · #92 15행 · #107 16행 여섯 증분을 여섯 재판정이 닫았다). 이 증분 16은 원장의 계수 규약(같은 범위의
증분 재판정은 원래 행의 결과 칸만 갱신)상 행 표에 보이지 않고 「전역 − 잔여」와 행 열 합의 차(2,271 ≠ 2,255)로만
...
**미판정 잔여**: **8파일 / 164행**
(이 패스 트리 기준 — 전역 `lines=2466 files=121`). 전역 지문의 파일 목록에서 판정 117파일을 **집합으로 뺀**
값이며, 뺄셈과도 일치한다(164 == 2,466 − 2,302). #107 이 들여온 새 파일 6개 중 마이그레이션 `0010` 하나만
잔여에 남았다 — 문서 편집 축이 「자매 슬라이스가 새 파일을 들여올 때마다 새 잔여가 생긴다」고 적어 둔
그대로이고(#92 → 0009, #107 → 0010), 남은 몫은 전부 사람 게이트다.

**잔여(행 없는 파일)는 네 몫뿐이고, 넷 다 사람 게이트다**(1 + 1 + 4 + 2 = 8파일 · 30 + 30 + 45 + 59 = 164행).
**무인 자유 풀은 비었다 — 판정 완료 범위 안의 미판정 증분 0행**(위 합계). 이 조건은 PR 번호에 묶이지
않는다 — **자매 모델의 슬라이스가 범위 안 파일을 다시 쓰거나 새 파일을 들여올 때마다** 증분·새 잔여가
생기고, #91·#99·#92·#107 이 그 형태였다. #92·#107 은 **새 파일**을 들여온 경우라 증분(판정 행 안)과
...
**다음 무인 패스의 후보 풀은 비어 있다**(#107 의 무인 167행은 이 패스가 닫았다 — 미판정 증분 0 · 새 무인
파일 0). 다음 후보는 자매 슬라이스가 판정 완료 범위 안에 새 주석을 더하거나 새 파일을 들여올 때 그
증분으로만 열린다. **「자유 풀」의 수치는 이 트리에서만 참인 조건부 진술이다** — 다음 감지는 절대 수치를
믿지 말고 **전역 지문의 파일 목록에서 위 117파일을 집합으로 빼서** 잔여를, 행 열의 합과 「전역 − 잔여」의
차로 미판정 증분을 다시 계산해야 한다. 증분 밖에서 진짜로 남은 일은 **마이그레이션 축의 다음 사람 게이트
패스**(0009 + 0010 60행 — 16차 패스와 똑같은 suspend · scale 0 · `UPDATE` · resume 를 한 번으로 요구한다)와,
...
행들에 다음 증분이 들어올 때 함께 집는다(이 패스는 새 파일 `sc03-03` 에만 최신 규약을 적용했다). D2·D6·D5
경로 벽 6파일 104행은 **주석 패스가 스스로 풀 수 없다**(판정기 자신을 고치는 것도 D6 다). 열린 PR #108(AC4.9)은
`backend/migrations/0010_llm_language.sql` · `migrations.rs` 만 건드려 이 패스의 11파일과 겹치지 않으나, main 에
이미 `0010_feature_additions.sql` 이 있어 같은 번호가 충돌한다 — 자매 축의 일이고 여기서는 관측만 한다.
