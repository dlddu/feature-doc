...
**합계**: 판정 **150파일**(지문의 파일 집합 기준으로는 145 — `format.ts` · `backend/tests/crypto.rs` ·
...
넘겼으므로 22차 패스가 이어 적었고, 이 줄은 29차 패스가 이어 적었고, 30차 패스가 **2**(행 열 안 미판정 증분 7행의 증분 재판정)를, 31차 패스가 **3**(원장 1행의 #49 경합 보류분 집행)을, 32차 패스가 **5**(행 10·11 에 #156 이 연 11행의 증분 재판정)를, 33차 패스가 **33**(#155 이 다시 연 43행의 판정)을, 34차 패스가 **53**(#158 이 연 124행의 판정 — 새 행 32 의 12 + 행 1·7·12·14 의 증분 재판정 41)을 이어 적는다) ·
판정 범위의 현재 합계 **2,792행**(행 열의 합 — 29차 패스가 **행 열 밖 잔여를 0** 으로, 30차 패스가
...
```
행 열의 합 2,792  +  ⑴ 행 열 밖 잔여 9  +  ⑵ 행 열 안 미판정 증분 13  =  전역 live 2,814
```

이 세 값은 **34차 패스의 트리**(PR head, 부모 `origin/main@174a29b`)에서 잰 것이다.
**⑴·⑵ 가 0 이 아닌 것은 이 패스가 남긴 몫이 아니다** — 34차 패스가 판정을 마친 뒤,
PR 을 열기 직전에 **#148**(`67c15ad`, e2e 폴링 단축)이, 머지를 기다리는 창에 **#161**
(`174a29b`, 목업↔구현 편차 원장)이 착지해 새로 열었다:
⑴ **9 = `e2e/support/clock.ts`**(#148 이 들인 신설 파일이라 판정 행이 없다) ·
⑵ **13 = 행 5 의 +5**(190 → live 195 — #148) **+ 행 11 의 +8**(117 → live 125 — #161 이
`tools/check-mockup-render.py` 의 「이 게이트가 보지 않는 것」 절에 연 8행). **둘 다 다음 감지
주기의 몫**이고, 34차 패스는
범위를 넓혀 이 둘을 삼키지 않았다 — 이 패스가 움직인 값은 **행 열의 합 2,721 → 2,792
하나뿐**이며 그 값은 #148·#161 착지 전후로 바이트 동일하다(둘 다 이 패스의 9파일 중 어느 것도
건드리지 않는다). 재는 법은
