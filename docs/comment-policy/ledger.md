...
0행이 되어 빠졌다. **파일 수는 지문의 `files` 가 아니라 행의 목록 길이로 센다**) · 순 제거 누적 **2,397행**
(직전 2,375 + 21차 패스(#139) 6 + 22차 패스 16) · 판정 범위의 현재 합계 **2,516행**(행 열의 합).
**그중 판정 완료는 2,477행**이고 **미판정 증분은 39행**이다 — #114 가 판정 완료 범위 안 네 행(원장 1·4·7·10행)에
...
밖이라 **잔여**로 들어갔다. 행 열의 합은 **2,516 = 판정 완료 2,477 + 미판정 증분 39**.

**22차 패스(이 패스)가 이 트리를 만들었다.** 19차 패스(#128)가 「벽은 걷혔지만 **판정 자체가 없다** …
...
`✅ 해당 없음`** 을 냈다. **이 패스만의 효과**는 `lines=2720 files=135` → `lines=2704 files=135` 다.
**이 트리의 검산은 행 열의 합 2,516 + 행 열 밖 미판정 증분 16 + 잔여 172 = 전역 2,704** 이다.

⚠️ **이 트리에서 행 5·7 은 재현되지 않는다 — 기록 오류가 아니라 #137·#138 의 미판정 증분이다.**
이 패스가 준비된 뒤 사람 PR **#137**(`fd6cdad`, 횡단 관심사 단계의 파일 앞부분 문맥)과
**#138**(`95d3395`, 경로 상한의 디렉터리 고른 샘플)이 판정 완료 범위 안 두 행에 **순 +16행**을 더했다 —
행 5 +2(`bin/worker.rs`) · 행 7 +14(`repo_scan.rs`). 증분 재판정은 **새 행이 아니라 원래 행의 결과 칸
갱신**이므로(「읽는 법」) 22차 패스는 이것을 섞지 않고 **다음 감지가 여는 몫**으로 넘긴다.
(같은 창에서 #132·#134 가 행 9·10 에 연 8행은 **21차 패스(#139, `6e6478a`)가 이미 닫았다** — 그래서
그 두 행은 이 트리에서 다시 재현된다.)
**나머지 21행은 줄 수·지문 둘 다 전건 재현된다**(실측 21/23 — 같은 파서를 base·head 양쪽에 굴려
미재현 집합이 {5,7} 임을 확인).
...
**미판정 잔여**: **7파일 / 172행**
(이 패스의 트리 기준 — 전역 `lines=2704 files=135` 에서 **행 열 밖 미판정 증분 16 을 뺀 2,688**).
전역 지문의 파일 목록에서 판정 133파일을 **집합으로 뺀** 값이며, 뺄셈과도 일치한다(172 == 2,688 − 2,516). #114 가 들여온 마이그레이션 `0013_feature_doc_conflicts.sql`
