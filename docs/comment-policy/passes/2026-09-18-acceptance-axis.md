...
## 증분 재판정 ⑥ (2026-09-26 · `rct_20260926-0002`) — 병합 호출 수 주석 3행, 전건 제거
...
| 자리 | 문면(요약) | 판정 | 근거 |
|---|---|---|---|
| `acceptance.rs` `derive` 의 `calls:` 앞 인라인 3 | 「저장소에 테스트 파일이 있으면 제공자 호출 두 번, 없으면 한 번 — 병합 답변은 한 행으로 저장되므로 행이 달리 보여 줄 수 없는 수를 그 행이 들고 간다(AC4.6)」 | **제거 3** | ① 바로 아래 `calls: logic.calls + tests.as_ref().map_or(0, |a| a.calls)` 가 「있으면 둘, 없으면 하나」를 **식으로** 말한다 · ③ #166 본문 「인수 시나리오 단계는 논리 호출과 테스트 호출 둘을 한 문서로 합쳐 **한 행만** 남기고(`acceptance::derive`), 테스트 경로가 없는 저장소에서는 한 번만 부른다」 **축자** · 뒤집으면 `a_merged_stage_row_counts_both_of_its_calls` 가 붉어지므로 조용하지 않다 · 이 명제의 **정본은 `llm.rs` 의 `Answer.calls` doc**(신설 행 34 의 「정본 지정」 표) |
