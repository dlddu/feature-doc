...
## 증분 재판정 ③ — `#107` 이 `FeatureCandidates.tsx` 에 더한 1행 (2026-09-21 · `rct_20260921-0010`)

`#107`(`89a1625`, 슬라이스 6b)이 `Props` 에 `onFinish` 를 더하며 쓴 JSDoc 1행 「미결정이 0건일 때 열리는
다음 단계 — 빠진 기능을 직접 더하는 화면(AC3.2)」을 판정해 **제거**했다. ① 같은 파일의 버튼
`disabled={list.undecided !== 0}` · `onClick={onFinish}` 와 카피 「결정 끝 — 빠진 기능 확인」이 그 문장
자체이고, 목적지는 `App.tsx` 의 라우팅(①)이 말한다 · ② doc-tracker 6b 행 「`FeatureCandidates` 의 `결정 끝
— 빠진 기능 확인` CTA 가 미결정 0 일 때 그리로 간다」 — 이 파일의 `onBack` 등 prop doc 을 「이름 재진술」로
걷은 4차·8차 판정과 같다. AC 꼬리표.
...
## 증분 재판정 ⑤ (2026-09-26 · `rct_20260926-0002`) — 후보 화면 누적 비용 2행
...
## E 표면 첫 판정 — `backend/tests/strategy.rs` 줄 끝 주석 1행 (2026-09-26 · `rct_20260926-0009`, 42차 패스)
...
- **①** 이 줄은 **반환값을 버린다**. 즉 호출의 목적이 값이 아니라 **부수 효과**(초안 적재)라는 것이
  주석에만 있다. 걷으면 다음 사람이 「쓰이지 않는 호출」로 읽고 지우게 되고, 그 뒤의 단정은 초안이
  없는 상태를 검사하며 **조용히 통과한다** — 「지우면 조용히 깨진다」형이다.
- **②** 판정 기록을 뺀 `docs/` 전역에 `materialise` **0히트**이고, PRD·테스트 문서는 초안이
  **언제 적재되는가**를 규정하지 않는다. **③** 저작 PR 2건(판정 패스 제외)에 이 호출의
  역할이 절로 적혀 있지 않다. **④ 공전**(최장 연속 일치 3자).
- 값: 줄 수·지문 **불변**(1 / `f28c846dc652bc87d558638aa45d2413eda0d667bdf8871a6dfdb87b1da4426f`) ·
  파일 **무접촉**. 판정 축 `—` → **`①②③④`**.
