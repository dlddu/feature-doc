...
19차 패스가 probe 로 확인한 것을 이 패스는 **실제 diff 로** 확인했다. 현재 main(`9acbd49`) 대비
...
- **주석 아닌 바이트가 부모와 동일 2/2** — 줄머리 `//`·`///`·`//!`·블록 주석을 걷어낸 잔여의 md5 가
  `backend/src/db.rs` · `backend/tests/migrations.rs` 둘 다 부모(`9acbd49`)와 같다.
  `APPLIED`·`PRE_CLEANUP` 의 값과 SQL 파일은 무접촉이다.
- `cargo test --manifest-path backend/Cargo.toml --profile ci` — **238 passed / 0 failed**(24 스위트).
  측정 출처는 PR head 의 CI 런 로그(`test (cargo)`)의 `test result:` 합이다. **#142 가 CI 의 프로필을
  `--release` 에서 전용 `ci` 프로필로 바꿨다** — 테스트 수는 프로필과 무관하다. 237 → 238 은 **#141** 이
  `backend/tests/github.rs` 에 `#[tokio::test]` 한 건을 더한 것이고(`82be2f4...c405ad7` 의 Rust diff 에서
  더해진 테스트 속성 1 · 지워진 것 0), 그 PR 의 CI 로그도 238 / 24 스위트를 냈다.
...
- 전역 주석 지문: 부모(`9acbd49`) `lines=2672 files=136` → **`lines=2656 files=136`** /
  `bc86dcd0c7a9e184345041ce65a6fdf95eb22bb1294b72b7abbf329de1b85fdf`
  (**순 제거 16행**, 파일 수 불변 — 주석이 0행이 된 파일이 없다).
  이 패스는 **아홉 base 위에서 측정됐고 순 제거 16 은 아홉 다 같다**: `b4a6b30`(#129 착지 전)
  `lines=2649` → `2633`, `27d9b81`(#129 착지 직후) `lines=2646` → `2630`,
  `074c325`(#131~#136 착지 후) `lines=2654` → `2638`, `95d3395`(#137·#138 착지 후)
  `lines=2726` → `2710`, `6e6478a`(#139 착지 후) `lines=2720` → `2704`,
  `82be2f4`(#142 착지 후) `lines=2721` → `2705`,
  `9b8effb`(#140 착지 후) `lines=2717` → `2701`,
  `c405ad7`(#145·#144·#141 착지 후) `lines=2732` → `2716`,
  `9acbd49`(현재 base — #143 착지 후) `lines=2672` → `2656`.
  **절대값은 자매 착지로 움직이고 순 제거는 안 움직인다** —
...
- **`e2e/tests/sc01-08-succeeded-stage-rerun.spec.ts` 13행** — #121(`b4a6b30`, AC1.5 확장)이 들여온
  **새 미판정 파일**이다. 이 패스의 측정점(`9acbd49`)에 이미 들어와 있지만 task 의 gap 이 지목한
...
- **판정 완료 범위 안의 미판정 증분 47행** — #121 이 이미 판정된 파일들에 더한 **39행**(20차 패스가
  ...
  뒤의 8행은 `rct_20260922-0009`(#139, `6e6478a`)가 **이미 착지시켰다** — 그래서 이 트리에서
  행 9·10 은 다시 재현되고, 남은 미재현은 #137·#138 이 연 행 5·7 뿐이다.
...
- **#131~#136**(사람 PR 5건)이 그 뒤에 착지했다. 그중 **#132·#134 가 행 9·10 에 연 8행**은
  **21차 패스(#139)가 이미 닫았다** — 그래서 그 두 행은 이 트리에서 다시 재현된다.
- 실행 창 안에 착지한 다섯 PR 중 **자매 주석 패스 둘이 미판정 증분을 대부분 닫았다** — **#145**(24차)가
  #142 의 행 14 +1 을, **#143**(23차)가 행 5·7·9 의 78행을 **증분 재판정**으로 닫았다(순 제거 60 —
  행 5 3 · 행 7 12 · 행 9 45). 그래서 **이 트리에서 미재현은 행 12 하나뿐이다**(22/23) — **#141** 이
  `github_app.rs`·`tests/github.rs` 에 연 **+14행**이고, 증분 재판정은 원래 행의 결과 칸 갱신이라 이
  패스에 섞지 않았다. 산술은 그 14행을 행 열 밖으로 세어 닫힌다 —
  **행 열의 합 2,466 + 행 열 밖 증분 14 + 잔여 176 = 전역 2,656**.
