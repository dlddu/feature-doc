...
또 #142(`82be2f4`, CI 축)가 `scripts/e2e.sh` 에 주석 1행을 더해 **원장 14행**(테스트 하네스 축,
20 → 21)이 기재값과 어긋난다. 14행은 이 슬라이스의 범위(5·7·9행)가 아니므로 **판정하지 않고 다음
감지에 넘긴다** — 이 PR 은 그 행을 건드리지 않는다.
