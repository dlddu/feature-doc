...
## 증분 재판정 ⑥ (2026-09-26 · `rct_20260926-0002`) — `sc01-05` 의 비용 단정 주석 2행

- **기준 트리**: 부모 **`0522f33`**(main, #172 착지 직후)
- **유입원**: PR **#166**(`ca9e14a`, 슬라이스 7d — AC4.6)
- **PR**: #NNN (37차 패스)

#166 은 이 자리의 옛 1행(「Cost is still the pre-flight estimate — measured spend is AC4.6.」)을
**실현된 예고**로서 걷고 2행을 새로 넣었다. 그 2행을 판정한다.

| 자리 | 문면(요약) | 판정 | 근거 |
|---|---|---|---|
| `sc01-05-resume-after-app-exit.spec.ts` `cost-so-far` 단정 위 2 | 「화면이 말하는 비용은 측정된 것이다(AC4.6) — 세 단계가 돌았으니 0 이 아니다. 얼마인지는 `sc04-09` 가 재고, 여기서는 『진행과 비용이 같은 서버 상태』만 본다」 | **제거 2** | ② `docs/doc-tracker/2026-09.md:100` 의 「자동화 밖 잔여」 칸이 **축자**로 갖는다: 「**실측 누적 비용**은 이 spec 이 「0 이 아니다」까지만 본다 — 얼마인지와 작업별·전체별 등식은 `sc04-09`(AC4.6)의 몫이다」 · ① 이 spec 의 단정 자신이 「0 이 아니다」까지만 재고, 파일 머리 3행(이 행 1차 판정에서 유지)이 이미 「진행은 서버 상태다」를 말한다 |

이 행의 줄 수 칸은 유입으로 **150 → 151** 이 됐다가 제거로 **149** 가 된다 — 1차 판정값보다 1 낮다.
판정 대상은 유입 2행뿐이고, 옛 1행은 #166 이 이미 걷었다(이 패스의 제거로 세지 않는다).

**순 제거 2행** · 줄 수·지문 150 → **149 / `77e8b096…`**

## E 표면 첫 판정 — `backend/src/cross_cutting.rs` 줄 끝 주석 1행 (2026-09-26 · `rct_20260926-0009`, 42차 패스)

#186 이 켠 E 표면에 이 파일의 줄 끝 주석 1행이 미판정 등재로 들어왔다. **유지 · 순 제거 0행.**

```
            "src/main.rs".to_string(), // duplicate — must be collapsed
```

- **①** 픽스처 목록에 같은 경로를 **두 번** 넣은 것이 의도라는 선언이고, 코드는 그 중복만 보여 준다.
  걷으면 다음 사람이 「중복 정리」로 한 줄을 지우고, 그때 접기(collapse) 단정은 **입력이 없어져 조용히
  공전한다** — 증분 재판정들이 유지로 판정해 온 「지우면 조용히 깨진다」형이고, `tools/` 체커 축의
  `open_tag` JSX 깊이 가드와 같은 잣대다.
- **②** 판정 기록(`docs/comment-policy/`)을 뺀 `docs/` 전역에 `must be collapsed` **0히트**.
  **③** 저작 PR 8건에 이 픽스처 설계가
  절로 적혀 있지 않다. **④ 공전**(최장 연속 일치 10자 ` — OpenAI `).
- 값: 줄 수·지문 **불변**(1 / `6de93a37bbaa8ac73c1af5ab0b9f49c292d66d17c1e772ac27aa3616af050b3d`) ·
  파일 **무접촉**. 판정 축 `—` → **`①②③④`**.
...
**증분 재판정 ①**(2026-09-20): #79가 `sc01-01`에 더한 순증 3행을 판정해 **순 제거 3행**(슬라이스 번호를 단 작업 흔적 2행 제거 · 목업 인용을 뺀 단정 의미 1행만 유지) · 범위는 145행 → 142행으로 돌아왔으나 **지문은 `fb27e0db…`가 아니라 `0e5c3d31…`**이다(살아남은 줄의 문면이 #79 이전과 다르다) — [passes/2026-09-20-pipeline-cross-cutting-axis.md](2026-09-20-pipeline-cross-cutting-axis.md)
