...
- #121 이 연 `backend/tests/progress.rs` 2행 — 9행의 「자매 착지 재실측」에 **이관으로 등재된** 항목이라
  이번 범위가 아니다.
- **착지한** PR **#137**(`fd6cdad`, `cross_cutting.rs` +278/-10) 이 들인 주석 30행과, 아직 열려 있는
  **#138**(+223/-29) 이 들일 주석. #137 의 훅은 모듈 머리(`//!` 3행 → 6행)와 `MAX_KEY_FILES`·`KEY_FILE_NAMES`
  의 새 doc·구분선 주석이라 **이 패스의 줄(20~22)과 겹치지 않았고**(hunk 실측: 1..12 · 51..56 · …),
  머지 시점 트리에서도 이 패스가 걷어낸 명제를 되살리지 않는다. 줄 수·지문은 위 「자매 착지 재실측」에서
  재고정했고, 그 30행의 **판정은 다음 감지의 몫**이다.
