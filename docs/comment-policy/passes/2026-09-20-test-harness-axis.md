...
> **규약 명시(2026-09-24 덧붙임 · `rct_20260924-0003`).** 위 두 값은 **전역 규약**(`printf '%s'` — 개행 없음)으로
> 계산한 것이다. 원장이 핀 칸에 쓰는 것은 **행 규약**(`printf '%s\n'` — 개행 포함)이라 같은 줄 집합이어도 값이
> 갈린다(원장 「행 지문을 재현하는 법」). 이 범위의 등재 커밋 `445ec57` 에서는 20행이
> **행 규약 `e8a9deb5…` / 전역 규약 `8b8764df…`** 로 둘 다 바이트 일치 재현된다. 원장의
> 2026-09-22 「지문 정정」이 이 값을 「한 번도 재현되지 않은 계산 오류」로 적은 것은 **규약을
> 맞추지 않은 탓**이며, 그 사유는 원장에서 정정됐다(핀 값 자체는 무수정).
...
## 증분 재판정 ① (2026-09-24 · `rct_20260924-0003`) — `scripts/e2e.sh` 1행

외부 기능 PR **#142**(`82be2f4`, 「ci: e2e 이미지 재사용 push, cargo ci 프로필, main 캐시 저장」)이
이 범위에 **순 +1행**을 열었다. 부모 `6e6478a` 20행 → HEAD `82be2f4` 21행이고, **행 델타 +1 ==
전역 델타 2720→2721 · 잔차 0** — 이 창의 유입이 이 한 줄뿐이고 다른 행은 무접촉임이 같이 증명된다.
...
### 판정 — 전건 제거 (복원 경로 넷 중 셋)

- **① 코드 자체** — `.github/workflows/ci.yml` 138~161행이 `Restore Playwright browsers`
  (`actions/cache/restore`, `~/.cache/ms-playwright`) → `Install e2e deps + Playwright chromium`
  (적중 시 `playwright install-deps chromium` 만) → e2e 스텝의 `SKIP_PLAYWRIGHT_INSTALL: '1'` 을
  그대로 적는다. **그 파일 자신의 137행 주석**이 같은 문장을 이미 담고 있다.
- **② 저장소 문서** — `README.md` 179행이 **같은 커밋에서** 「Playwright chromium(main 이 저장한
  브라우저 캐시를 복원, 적중 시 apt 의존성만 설치) → `SKIP_BUILD=1 SKIP_PLAYWRIGHT_INSTALL=1
  scripts/e2e.sh`」로 갱신됐다.
- **③ PR 본문** — #142 §4 가 동일한 내용을 축자로 서술한다.

줄 자체는 **호출자가 이 플래그를 왜 켜는지**만 말하고, 플래그가 *무엇을* 하는지는 바로 아래 선언과
`if [ "${SKIP_PLAYWRIGHT_INSTALL}" != "1" ]` 블록이 이미 말한다(유형 ① 선언 재진술 + ② 문서 재진술).
상류 API 의 미문서화 동작 · 저장소 제약 · 동시성 계약 · 실패 모드의 함정 · stub↔real 충실도 경계
어느 항목에도 해당하지 않아 「애매하면 남긴다」의 보호 대상이 아니다.

**대조군이 같은 파일 안에 있다.** 60·61행의 `kubectl rollout` 조기 반환 주석(이 패스가
「유지한 것」으로 남긴 2행)은 **실패 모드의 함정**이라 유지가 맞다. 이번 줄은 그 성격이 없다.

### 값 — 재고정이 아니라 원복

| 시점 | 줄 수 | 행 규약(`printf '%s\n'`) | 전역 규약(`printf '%s'`) |
|---|---|---|---|
| 부모 `6e6478a` | 20 | `e8a9deb55b008ab2f51615e749372a44ceb40cd2c200be800852a7b38c18017a` | `8b8764df56b28c69c1419f82a0f65c23c0316815315a37e6b0e9d09af544fc9c` |
| HEAD `82be2f4` | 21 | `5f016e49ad14dd9e8e2bcd8ca20d11f3337215a95b699a30064c776f231d3cc6` | `5cfdfcea05c96cb2b40b101c1f38a1f9b2f16d98dbbbdc4a233100957c31374d` |
| 이 패스 | 20 | `e8a9deb5…`(부모와 바이트 동일) | `8b8764df…`(부모와 바이트 동일) |

제거하면 두 규약 모두 부모 값으로 돌아오므로 **원장의 줄 수·지문 칸은 손대지 않았다.**
완료 기준은 절대 지문이 아니라 **「부모 대비 순 제거 1행」**이다 — 자매 PR 이 먼저 착지해도
이 5파일을 건들지 않는 한 불변이다.

### 소유자 0 · 경합 0

열린 PR 6건(#144·#143·#141·#140·#130·#126)의 파일 목록 전수에서 **`scripts/e2e.sh` 를 만지는 PR 0건**,
이 패스 파일을 만지는 PR **0건**이다. `docs/comment-policy/ledger.md` 는 #143(원장 5·7·9행)·#130(자유 풀 행)과
공유되지만 헝크가 같은 행에 닿지 않는다(이 패스는 13·14행, 저쪽은 5·7·9행과 자유 풀 행).

### 덧붙인 정정 — 원장 13·14행의 「등재 시점 계산 오류」는 규약 혼동이었다

원장의 2026-09-22 「지문 정정」이 두 행에 똑같은 사유를 적었다 — 「등재 커밋에서부터 한 번도
재현되지 않았다 → 등재 시점 계산 오류」. **둘 다 사실이 아니다.** 두 값 모두 자기 등재 커밋에서
**전역 규약으로 바이트 일치 재현된다**:

| 원장 행 | 등재 커밋 | 줄 수 | 행 규약 | 전역 규약 = 「재현 안 됨」이라 된 값 |
|---|---|---|---|---|
| 13행 (자격증명·LLM 키 경계 8파일) | `7415771` | 42 | `1823c6f1…` | **`96636e79…`** ✓ 재현 |
| 14행 (테스트 하네스 축 5파일) | `445ec57` | 20 | `e8a9deb5…` | **`8b8764df…`** ✓ 재현 |

줄 집합도 값도 틀리지 않았고 **규약만 섞였다.** 원장이 머리에 「재현이 안 맞으면 먼저 이 개행부터
의심한다」고 써 둔 바로 그 함정에, 정정 자신이 빠졌다. **핀 값(행 규약)은 맞으므로 그대로 두고
사유만 정정했다** — 발견 기록은 지우지 않는다.

14행의 정정문이 13행을 번호로 인용하므로(「13행과 같은 …」) 두 행을 함께 정정했다 — 한 쪽만 고치면
인용된 근거가 거짓인 채로 남는다. **16행의 「#98 은 리베이스 전 계산」은 다른 종류의 주장이라
이 패스가 재지 않았다** — 미측정으로 남긴다(범위 밖).

### 검증 (이 패스 트리, 부모 `82be2f4`)

- **범위 지문 원복 2규약** — 위 표의 세째 행. `e8a9deb5…` · `8b8764df…` 둘 다 부모 `6e6478a` 와 바이트 동일.
- **`scripts/e2e.sh` 로직 무변경** — `git diff` 가 주석 1행 삭제만 보인다(`1 file changed, 1 deletion(-)`).
  변수 선언 블록은 부모와 같은 배치로 붙고, 빈 줄은 건들지 않았다.
- **`bash -n scripts/e2e.sh`** rc=0.
- **`check-data-format-change.py --base <main tip> --head <head>`** → `✅ 변경 없음` (`review/manual-approval` 자동 success).
- **문서 게이트 3종 rc=0** — `check-journey-mockup.py`(R0~R11 — 새 `.md` 가 없어 허브 R9 불변) ·
  `check-mockup-render.py` · `check-scenario-e2e.py`.
...
## 증분 재판정 ⑪ (2026-09-26 · `rct_20260926-0002`) — `scripts/e2e.sh` 샤딩 변수 2행

- **기준 트리**: 부모 **`0522f33`**(main, #172 착지 직후)
- **유입원**: PR **#169**(`21e2576`) · **#170**(`87835d2`) — Playwright 샤딩
- **PR**: #NNN (37차 패스)
