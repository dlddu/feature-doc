...
## 증분 재판정 ①(② 축) — 2026-09-26, 44차 패스 (`rct_20260926-0011`)

16차 패스가 이 행을 `①③④`로 닫고 **② 저장소 문서 축을 묻지 않았다**. 그 뒤 다섯 달치 슬라이스가
`docs/doc-tracker/`에 자기 설계 판단을 적어 두었으므로, 이 패스가 그 축을 행 전체에 대해 물어
행을 **`①②③④`**로 닫는다. 지문·줄 수는 불변(**123행 / `1b35c33a8217…`**)이다 — **이 PR 은 `.sql` 을
한 글자도 건드리지 않는다.**

### 왜 이 패스에서 `.sql` 을 건드리지 않는가

정책 본문 「적용된 마이그레이션의 주석」 절은 네 항을 세는데, **1항(판정)만 「다른 파일과 같다」**고
적고 2항(전용 PR)·3항(repair 명세)·4항(사람 게이트)은 **집행**을 가둔다. 판정이 `.sql` 을 건드리지
않으면 `tools/check-data-format-change.py` 의 D1(`f.startswith("backend/migrations/")`)이 이 PR 의 실제
diff 에 발화하지 않아 `review/manual-approval` 이 자동으로 붙는다 — 27차 패스(#150, `0009`~`0013`)와
31차 패스(`0014`)가 이미 같은 경로로 닫았다. 열 패스째 「범위 밖(사람 게이트)」으로 이월되던 이 축이
그렇게 무인으로 움직였다.
...
| 자리 | 명제 | 판정 | ② 근거 |
|---|---|---|---|
| `0004:1-3` | 워커가 이 DB 를 열지 않아 API 가 단일 writer 로 남고 그 위에 선 SQLite 불변식(한 파드·한 마운트·`Recreate`)이 그대로 산다 | **제거 후보 3** | `docs/doc-tracker/2026-08.md:13` 축자(「워커는 데이터베이스를 열지 않고 API의 `/internal` 라우트로 … SQLite가 요구하는 단일 writer(`replicas:1` + `Recreate`)를 깨지 않으면서」) · `README.md:124` · `backend/migrations/README.md:113` — **세 벌** |
| `0005:7-11` | `content_hash` 가 결정성 요구를 관측 가능하게 만든다 | **제거 후보 5** | `docs/doc-tracker/2026-08.md:16`(「결정성은 대신 **관측**한다 … 산출물의 `content_hash`를 직전 동일 대상 분석과 대조해 `first`/`unchanged`/`changed`를 명시한다」) |
| `0006:5-9` | 제안(문서)과 승인본(테이블)을 일부러 두 곳에 나눴다 | **제거 후보 5** | `docs/doc-tracker/2026-08.md:17` 축자(「설계의 핵심은 **제안과 승인본을 두 곳에 나눈 것**이다 … 사람이 한 줄 지워도 "모델이 무엇을 제안했는가"가 변하지 않고, 단계 재시도가 사용자의 편집을 지우지 않는다」) |
| `0006:17-19` | `approved_at IS NULL` 이 초안이고 승인 전까지 claim 이 다음 단계를 큐에서 뺀다 | **제거 후보 3** | 같은 행(「「승인된 전략만 다음 단계의 입력이 된다」는 산문이 아니라 큐의 성질로 표현했다 — `worker_api::claim` 이 승인 전까지 `feature_candidates` 를 `executableStages` 에서 빼고」) |
| `0007:14-18` | `key` 는 행 id 도 이름도 아니고 발견 위치에서 파생한, 분석을 가로지르는 정체성이다 | **제거 후보 5** | `docs/doc-tracker/2026-09.md:714` 축자(「후보의 신원은 **행 id 도 이름도 아닌 발견된 위치**(`location`[`#symbol`])다 — 이름은 사용자가 제일 먼저 바꾸는 값이고 … 다음 분석은 같은 자리에 새 행을 쓰므로 위치만이 「같은 후보」를 말할 수 있다」) |
| `0007:20-22` | 거부는 사유를 남겨야 하고 CHECK 가 두 번째 방어선이다 | **제거 후보 3** | 같은 행(「거부 사유는 라우트에서 400 으로, 테이블에서 CHECK 로 두 번 강제한다」) |
| `0008:7-10` | 답해야 하는 것이 행 선택 질의라 JSON blob 이 아니다 — `0007` 이 후보를 행으로 고른 것과 같은 이유 | **제거 후보 4** | `docs/doc-tracker/2026-09.md:725` 축자, **교차 참조까지 같다**(「적재는 문서가 아니라 행이다 … 행 선택 질의라 JSON blob 으로 두면 모든 분석의 blob 을 파싱해야 답이 나온다(**0007 이 후보를 행으로 고른 것과 같은 사유**)」) |
| `0008:18-21` | `category` CHECK 가 코드 카테고리 상수의 두 번째 방어선이다 | **제거 후보 4** | 같은 행(「분류 7종의 단일 정의는 `dependencies::CATEGORIES` 이고 마이그레이션 CHECK·프롬프트·화면 칩이 전부 그 하나를 본다」) |
| `0008:23-25` | `evidence` NULL 은 결함이 아니라 기록된 사실 — 「근거 없음」으로 명시하고 임의로 채우지 않는다 | **제거 후보 3** | `docs/user-journey/JRN-review-feature.md:88` 축자(「의존성 근거가 비어 있음 / "근거 없음"으로 명시. 임의로 채우지 않음」) · `docs/prd/03-doc-management.md:25` · `docs/doc-tracker/2026-09.md:725` — **세 벌** |
| `0001` 전건 11 · `0002` 전건 4 · `0003` 5 · `0004:5-6`·`:14`·`:17-19` · `0005:1-5`·`:13-14` · `0006:11-15` · `0007:1-4`·`:6-12`·`:24-27`·`:49-50` · `0008:1-5`·`:12-16`·`:27-31`·`:70` | — | **유지 88** | ② 로 복원되지 않는다. 아래 두 실측이 근거다 |

**합계: 제거 후보 35행 · 유지 88행 · 순 제거 0행**(집행은 repair 창 PR).

### 유지 88행의 근거 둘 — 「없음」을 건수로 쟀다

⑴ **② 자신이 그은 소유 경계.** `backend/migrations/README.md` §「주석에 무엇을 쓰나」가 `.sql` 주석을
「이 변경을 **왜** 했는가 — 그 시점의 기록」의 자리로 배정하고, §「시점 서술을 쓰지 않습니다」가
「스키마의 **지금 의미**」를 코드 옆 rustdoc 으로 배정한다. 즉 이 배정 안에 남는 문장은 **저장소 문서가
자기 소유가 아니라고 선언한 것**이라 ② 로 복원될 수 없다. 이것이 「유지」를 지어내지 않는 판별식이다 —
② 히트 아홉 자리는 그 배정을 넘어 **doc-tracker 가 같은 「왜」를 되풀이한** 자리들이다.

⑵ **md 전수 grep 0건**(`git ls-files '*.md'` − `node_modules` − `docs/comment-policy/`, 64파일):
`AES` **0** · `DEK` **0** · `KEK` **0** · `epoch` **0** · `uuid` **0** · `spoofable`/「스푸핑」 **0**.
그래서 `0001` 11행(다중 사용자 소유 규약 · epoch 초 · uuid v4 · 세션 토큰 비가역 · 봉투 암호화 스킴
2단 · append-only 감사)과 `0002` 4행(Setup URL 의 `installation_id` 가 spoofable 이라 OAuth 토큰으로
소유를 검증한다)이 **전건 유지**다. `docs/prd/04-platform.md:29` 는 「저장 시 암호화 · 필요한 호출 직전에만
복호화 · 로그 출력 금지」까지만 말하고 **스킴(AES-256-GCM · per-record DEK · process KEK)을 말하지 않는다** —
16차 패스가 ① 축에서 `0002` 를 「한 글자도 안 바꿨다」로 닫은 것과 같은 결론이 ② 에서도 성립한다.

### 게이트·무영향

- `.sql` **8파일 무접촉** — 이 패스의 diff 는 `docs/comment-policy/` 뿐이라 D1 이 발화하지 않는다.
  행의 줄 수·지문은 **불변**(123 / `1b35c33a8217…`)이고 I3 가 그것을 집행한다.
- `python3 scripts/check-comment-ledger.py` **rc=0** — `[L] 판정 완료 2721줄 / 35행 (90%)` →
  **`2888줄 / 38행 (96%)`**, 축별 미판정 행 `① 3 · ② 4 · ③ 3 · ④ 4` → **`① 2 · ② 2 · ③ 2 · ④ 2`**.
- 셀 인덱스 diff 검산: 고친 세 행이 각각 `[1, 5, 6]`(판정일 · 판정 축 · 결과 요약)뿐이고 나머지 37행은 0건.

### 이월 — 집행은 한 repair 창으로

본문 1항이 「repair 비용이 크므로 한 패스에서 마이그레이션 판정을 끝까지 모아 한 번의 repair 로
끝낸다」고 못박는다. 지금 등재된 제거 후보는 **세 묶음**이다:

| 출처 | 자리 | 기대 순 제거 |
|---|---|---|
| 27차 패스 (`0009`~`0013`) | 「제거 후보 검토」 세 자리 | 6~8행 |
| 31차 패스 (`0014`) | `:1` · `:3-5` · `:19-23` | 9~11행 |
| **이 패스** (`0001`~`0008` ② · `0015` 전축) | 위 표 아홉 자리 + `0015:1`·`:3-10` | 45행 |

세 묶음이 **한 전용 PR · 한 repair 창**의 몫이고, 그 PR 은 이미 줄 번호까지 적힌 목록을 집행만 하면
된다 — 다시 세지 않는다.
