...
- **`backend/src/llm.rs:494-499` 의 주석이 거짓이고, 그 거짓이 실제 flake 를 덮고 있다.**
  ...
  기록만 한다.
  **— 해소됨(2026-09-22 · PR #123 `77158c2` · `rct_20260922-0001`).** 그 별개 작업이 수행돼 레이스가
  사라졌고 거짓 두 절이 정정됐다. 유입 4행의 판정은
...
## E 표면 첫 판정 — `backend/src/session.rs` 줄 끝 주석 1행 (2026-09-26 · `rct_20260926-0009`, 42차 패스)

#186 이 켠 E 표면에 이 파일의 줄 끝 주석 1행이 미판정 등재로 들어왔다. **전건 제거 1행.**

```
const SESSION_TTL_SECS: i64 = 60 * 60 * 24 * 30; // 30 days
```

- **①** 상수명이 `_SECS` 로 단위를 들고 있고 `60 * 60 * 24 * 30` 이 초→분→시→일→30 을 그 순서로
  적는다. 곱을 접지 않고 남겨 둔 형태 자체가 「30 일」이라는 문장이다.
- **②** `docs/prd/04-platform.md:54` 는 「만료가 있는 HttpOnly 쿠키」까지만 적고 **기간을 규정하지
  않는다** — ② 로는 「30 일」이 복원되지 않는다(이 줄은 ① 로 닫힌다).
  **③** 저작 PR 1건(판정 패스 제외)에 이 값의 근거가 없다. **④ 공전** — 그 커밋의 트레일러 밖 본문이
  0자이고 최장 연속 일치 4자(` key`).
- ①만으로 닫히므로 남은 셋이 무발화여도 제거다(복원 경로는 「넷 중 어디서든」이다).
- 값: 원장 E 행 1 → **0 / `01ba4719c80b6fe911b091a7c05124b64eeece964e09c058ef8f9805daca546b`**.
  이 행은 **이 표면에서 0행이 된 등재 행**으로 남는다(판정이 있었다는 기록). 이 파일의 L 행은 다른
  덩어리 소속이고 무접촉이다. 판정 축 `—` → **`①②③④`**.
