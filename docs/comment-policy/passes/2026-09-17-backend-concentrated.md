...
## E 표면 첫 판정 — `backend/src/analysis.rs` 줄 끝 주석 3행 (2026-09-26 · `rct_20260926-0009`, 42차 패스)

#186 이 켠 E 표면에 이 파일의 줄 끝 주석 3행이 미판정 등재로 들어왔다. 이 행의 L 판정(2026-09-17 및
증분 ①~⑮)은 줄머리 주석만 보았으므로 여기가 첫 판정이다. **전건 유지 · 순 제거 0행.**
...
- **①** 세 줄은 스텁 비용 추정의 **상수 근거**다. 코드는 `3`·`8`·`10`·`6/10` 이라는 값만 들고 있고
  「한 소스 파일이 대략 3 KiB」·「스캔은 배치로 묶이고 파이프라인 고정 호출이 10건」·「호출당 약
  $0.006」이라는 **모델링 전제**는 어디에서도 읽히지 않는다. 모델 「유지 대상」의 **복원 불가능한
  지식**(값이 왜 그 값인지)에 해당한다.
- **②** `docs/` 에서 이 모델의 판정 기록(`docs/comment-policy/`)을 뺀 전역에 `3 KiB`·`0.006`·
  `per source file` **0히트**(실측 — 판정 기록 안의 히트는 이 절이 그 문면을 인용해 생긴 것이라
  복원 경로가 아니다).
- **③** 저작 PR 18건(주석 축 판정 패스 11건 제외) 어디에도 이 셋의 산술 전제가 없다.
- **④ 공전** — 잔여 255행의 최장 연속 일치 **10자**(` analysis `).
- **판단이 갈린 한 줄**: `// ~$0.006 per call` 은 `cost_cents = llm_calls * 6 / 10` 에서 산술로
  환산되므로 ① 쪽으로 기운다. 그런데 세 줄은 **한 가족**이고(같은 추정식의 세 항), 하나만 걷으면 남는
  둘의 단위 대응이 반쪽이 된다 — 「3 KiB」와 「$0.006」이 짝을 이뤄 「KiB → 호출 수 → 비용」 사슬을
  말한다. 본문 「충돌 시 기본 방향」의 **「애매하면 남긴다 … 그 사실을 `passes/` 에 남긴다」**를
  적용한다.
