...
- 화면 10개(S01~S10) 중 등장: **10개** ✅
- **시각화 공백 1건**: `JRN-restore-history` 전체 (이력 화면 미제작, S11 후보). [`doc-tracker.md`](../doc-tracker.md)에 수용된 위험으로 등재되어 있습니다. `JRN-connect-repo` / `STP-sign-in`의 미인증 상태는 여정 페이지가 실제 로그인 화면을 가지면서 해소됐습니다
...
2. **화면 추가·제거** → 영향받는 단계의 터치포인트와 §5 매트릭스 갱신. 여정 문서의 구조는 바뀌지 않습니다 (화면은 단계가 아니므로)
