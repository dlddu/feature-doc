...
- **Mockup** — 디자인 톤·색·폰트·인터랙션이 모두 결정된 최종 형태. 디자인 시스템의 표현 결과물. 화면별 HTML 목업은 [`docs/mockups/`](../mockups/)에 있어요.
- **Wireframe** — 정보 구조와 화면 흐름의 합의. 디자인 톤이 바뀌어도 기능 변경이 없으면 그대로 유지됨.

따라서 PRD가 변경되거나 화면 흐름을 재검토할 때 와이어프레임을 먼저 갱신하고, 그 다음 [`mockups/`](../mockups/)를 디자인 시스템 토큰으로 다시 그립니다.
