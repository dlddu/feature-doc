...
1. **더블 활성 배선** — 아래 5개 env 중 하나를 `stub`으로 켜는 자리(`deploy/e2e/kustomization.yaml`의 API·워커 패치). 전역 모드 스위치는 2026-09-18 제거됐다(아래 「stub mode 제거 — 완료 기록」). 배선은 **모킹 지점이 아니다**: 등재 대상은 그것이 선택하는 코드 분기이고, 표기 규약대로 활성 지점에는 예외 주석을 달지 않는다.
   ...
   각 env는 **그 경계를 소유한 프로세스만** 읽는다. LLM 경계만 두 프로세스가 소유한다 — 분석 파이프라인은 워커가, AC3.1 편집 제안은 큐를 타지 않고 API가 직접 모델을 부르므로(2026-09-21, #92) **같은 이름을 각자의 프로세스에서** 읽는다. 값이 아니라 이름을 공유하는 것이라 한쪽만 stub인 배포가 가능하고, 오버레이의 API 패치와 워커 패치가 이 env를 따로 켠다. 그 밖의 경계는 API Deployment에 워커 더블을 켤 수단이 없고 그 역도 같아서, 「하나를 켜면 전부 켜진다」는 성질이 배선 수준에서 사라졌다. 미설정은 언제나 실연동이므로 아무 말도 하지 않는 배포는 아무것도 stub하지 않는다.
2. **stub 분기** — `Mode::Stub` 분기가 외부 경계를 결정적 더블로 대신하는 자리(GitHub 로그인 신원 · App 설치와 설치 토큰 · 접근 가능 저장소 목록 · 저장소 트리 스캔 · LLM 호출 · LLM 키 실검증), 그리고 LLM 단계가 고정 답을 공급하는 자리(`Ask { stub: stub_*(...) }` — 현재 `stub_answer` · `stub_logic` · `stub_tests` · `stub_dependencies` · `stub_edit`).
...
## 허용목록 (등재 집합 — 2026-09-21 증분 등재, 16쌍 / 13파일)

지점은 **파일 × 토큰 쌍** 단위로 등재한다. 아래 표가 등재 집합이며, 각 행의 CODE는 코드 주석에 동일하게 부착된다. 2026-09-21 AC3.1 편집 제안의 고정 답 공급이 등재된 뒤로 **코드 실재 지점 16쌍 == 등재 16쌍**이다(미등재 0 · 고아 0). 「활성 env」 열은 그 행의 더블을 켜는 배선이며, 배선 자체는 등재 대상이 아니다.
...
| 16 | `backend/src/doc_edit.rs` | `stub: stub_edit` | LLM-01 | `FEATUREDOC_DOUBLE_LLM`(API) | 편집 제안(AC3.1) Ask의 고정 답 공급(제품 경로 — 워커가 아니라 API 프로세스가 직접 부르는 유일한 LLM 지점) |
...
결과다. 현재 수치는 위 허용목록 절이 갖는다 — 2026-09-19 종단 의존성 추출 1쌍, 2026-09-21 AC3.1 편집 제안 1쌍이 더해져 16쌍이다).
