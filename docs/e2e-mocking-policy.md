...
|----|-----------|-----------|------|------|-------------|
| R1 | stub LLM은 실제 한도 초과·시간 초과로 실패하지 못해, 제품의 LLM-기원 실패 분기(단계 실패·사유 표시·부분 재시도)가 E2E에서 LLM 오류로는 한 번도 밟히지 않는다(sc01-06은 트리 404로 같은 분기를 밟는 우회로 쓴다) | 등재가 아니라 충실도 확장으로 닫는다: `llm::stub_answer`에 결정적 실패 트리거를 추가해(예: 특정 입력에서 real과 같은 오류로 실패) sc01-06이 LLM 실패를 직접 재현하게 한다 | feature-doc 백엔드 stub 구현(`backend/src/llm.rs`)과 `e2e/tests/sc01-06-partial-retry.spec.ts` | 없음(착수 가능) | 2026-12-16 |
