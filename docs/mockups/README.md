...
- **Format** — HTML, 다크 테마, 반응형. 기기 프레임 없이 **뷰포트 전체가 앱 창**(`.frame`)이라, 폰에서 열면 폰 앱 화면, 데스크톱에서 열면 데스크톱 웹 화면이 실제로 보일 모습 그대로 나옵니다(상태바·홈 인디케이터·둥근 테두리를 그리지 않습니다). 한 파일이 앱 창 폭에 따라 [`design-system.md` §3.4](../design-system.md#34-responsive-layout) 의 세 가지 배치(Compact · Medium · Expanded)로 렌더되고, 폭 판정은 `.frame` 을 앱 창으로 보는 container query 입니다. 보조 레이어(「여정 정보」)는 앱 창 밖 화면 맨 아래 한 줄(`--doc-bar`)에 있고, 열면 위로 펼쳐집니다. 그 안의 **미리보기 폭** 스위치로 뷰포트와 무관하게 폰 393 · 태블릿 834 · 데스크톱 1280 배치를 오갈 수 있으며, 이때만 창 경계가 1px hairline 으로 보입니다. 새 목업도 이 반응형 블록과 스위치를 그대로 포함하고, 반복 카드 목록 컨테이너에는 `class="collection"` 을 답니다.
