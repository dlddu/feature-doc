...
아래 묶음(01~04)은 **화면 단위** 그룹입니다. 목업은 와이어프레임과 1:1로 대응해요.

사용자 여정 문서는 화면이 아니라 **행동** 단위(`JRN-`/`STP-`)로 정리되어 있어, 이 묶음과 1:1로 맞물리지 않습니다. 어느 여정이 어느 화면을 터치포인트로 쓰는지는 [`user-journey/README.md` §5 연결 매트릭스](../user-journey/README.md#5-연결-매트릭스--여정--가치--ac--와이어프레임)를 보세요. "여정 하나 = 목업 페이지 하나"로 전환하는 건 아직 미결정 사항입니다 ([`doc-tracker.md`](../doc-tracker.md) 참조).
