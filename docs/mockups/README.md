...
- **Format** — HTML, 다크 테마, 반응형. 한 파일이 앱 창 폭에 따라 [`design-system.md` §3.4](../design-system.md#34-responsive-layout) 의 세 가지 배치(Compact · Medium · Expanded)로 렌더됩니다. 폭 판정은 `.frame` 을 앱 창으로 보는 container query 라서, 뷰포트가 600px 이상이면 `.frame` 이 폰이 아니라 앱 창(최대 1280 × 860, 14px radius)으로 커지고 안쪽이 그 폭의 배치로 흐릅니다. Compact 는 여전히 393 × 844 iOS 프레임이 **상한**이고, 460px 이하 뷰포트에서는 프레임(보더·38px radius·상태바·홈 인디케이터)을 유지한 채 `min()` 으로 뷰포트에 맞춰 줄어듭니다 — 실기기에서 가로 스크롤이나 이중 스크롤이 생기지 않게 하기 위한 것이에요. 보조 레이어(「여정 정보」)의 **미리보기 폭** 스위치로 뷰포트와 무관하게 폰 393 · 태블릿 834 · 데스크톱 1280 배치를 오갈 수 있습니다. 새 목업도 이 반응형 블록과 스위치를 그대로 포함하고, 반복 카드 목록 컨테이너에는 `class="collection"` 을 답니다.
...
4. **값과 잇는 말이 한 문장에 섞여 있으면, 잇는 말도 각자 잎으로 감쌉니다.** 체커는 예시값
   요소를 *내용째 걷어 내므로*, 값만 빼면 그 자리에서 앞뒤 텍스트가 한 덩어리로 붙습니다 —
   `… 코드는 <값>. 테스트는 <값>고 적혀 있어요.` 에서 값만 빼면 대조 대상이
   `코드는 . 테스트는 고 적혀 있어요.` 라는, **구현이 결코 만들 수 없는 문자열**이 됩니다.
   잇는 말을 `<span>` 으로 감싸 두면 태그가 경계로 남아 `코드는` · `. 테스트는` ·
   `고 적혀 있어요.` 셋으로 갈라지고, 구현이 같은 조각을 그대로 그릴 수 있습니다. 감싸기는
   인라인이라 **렌더 결과는 한 글자도 바뀌지 않습니다**(도입: 2026-09-02 슬라이스 5a,
   `JRN-review-feature.html#STP-read-scenarios`).

> 이 속성은 렌더링에 영향을 주지 않습니다(스타일 훅이 아닙니다). 목업을 `file://` 로 열었을 때
> 보이는 화면은 붙이기 전과 완전히 같습니다.

## 선택 변이 표기 규약 (`data-variant`)

예시값과 성격이 또 다른 세 번째가 있습니다 — **사용자의 선택에 따라 문면이 갈리는 자리**입니다.
LLM 제공자를 고르면 키 형식 안내와 입력 예시가 함께 갈립니다. 정적 HTML 은 그런 자리에 첫
변이 한 벌만 그릴 수 있고(나머지는 프로토타입 script 안에만 있습니다), 구현은 선택을 따라
보간해 그립니다. 그래서 리터럴 대조가 **한 방향으로만** 성립합니다.

- 목업이 그린 `Anthropic 키는` 이 구현에 리터럴로 없는 것은 이탈이 아닙니다 — 구현은 고른
  제공자의 한 벌만 그립니다. 모든 변이가 구현에 실재하기를 요구하면 그 자리는 영원히 붉습니다.
- 반대로 구현이 그리는 `AIza` 가 목업 어디에도 없으면 그것은 **이탈입니다** — 목업이 열거하지
  않은 문면을 제품이 만들어 낸 것이기 때문입니다.

`data-variant="<축>"` 은 그 비대칭을 그대로 표기합니다. 붙은 요소는 **M3A(목업→구현)에서
빠지고, M3B(구현→목업)의 대조 집합에는 남습니다.**

```html
<span id="key-hint-anthropic" class="key-hint" data-variant="provider">Anthropic 키는 <code>sk-ant-</code>로 시작합니다.</span>
<span id="key-hint-openai" class="key-hint" data-variant="provider" hidden>OpenAI 키는 <code>sk-</code>로 시작합니다.</span>
<span id="key-hint-google" class="key-hint" data-variant="provider" hidden>Google 키는 <code>AIza</code>로 시작합니다.</span>
```

**이것은 면제가 아니라 열거 의무입니다.** `data-sample` 과 갈리는 지점이 여기입니다 — 예시값은
양방향에서 빠지지만(구현도 그 리터럴을 갖지 않습니다), 변이는 구현이 그중 **하나를 리터럴로
갖습니다**. 그래서 목업이 적어 둔 변이 집합이 곧 **구현이 그려도 되는 것의 전부**가 됩니다.

**규칙 세 가지.**

1. **변이는 정적 HTML 에 다 열거합니다.** script 가 문면을 조립하면 정적 파일에 첫 한 벌만
   남아 게이트가 나머지를 보지 못합니다 — 이 규약이 닫으려는 바로 그 구멍입니다. 프로토타입은
   열거된 것 중 하나를 **보이고 감추는** 방식으로 동작시킵니다(렌더 결과는 그대로입니다).
2. **한 축에 변이가 하나뿐이면 안 됩니다.** 같은 단계 안에서 같은 `data-variant` 값이 2개
   미만이면 그것은 열거가 아니라 한 벌을 숨기는 것이며, 게이트 규칙 **M6** 이 실패시킵니다.
3. **전진 행동에는 붙일 수 없습니다.** `data-goto`·`data-goto-journey`·`data-cta` 를 가진
   요소는 금지입니다 — 무엇을 눌러 다음 단계로 가는지는 선택과 무관한 제품 카피입니다.
   이 금지도 **M6** 이 집행합니다.

> 한계를 적어 둡니다: 규칙 2 는 **개수**만 셉니다. 한 축에 성격이 다른 두 요소가 있으면
> (안내문 `<span>` 3벌 + 입력 `placeholder` 1개처럼) 개수는 채워지므로, 부분 열거까지
> 기계가 잡아내지는 못합니다. 그 자리를 실제로 지키는 것은 M3B 입니다 — 열거되지 않은
> 문면을 구현이 그리는 순간 붉어집니다. (도입: 2026-09-20 슬라이스 ⑧,
> `JRN-connect-repo.html#STP-register-llm-key`)
...
([`doc-tracker/`](../doc-tracker/index.md) 「알려진 목업↔구현 편차」의 유형 **「제3자 소유」**).
...
여정 문서에서 파싱한 값입니다. 「예외」는 [`doc-tracker/`](../doc-tracker/index.md) 「수용된 위험」에 여정 단위로 등재되어 목업 페이지가 없어도 되는 여정입니다.
...
>
> **커버리지: §4 컴포넌트 12개 중 11개 사용 · 미사용 1개(`Tabs` §4.6).** `Tabs` 는 전 목업의 인라인 CSS 에 정의만 있고 마크업에서 한 번도 쓰이지 않습니다. 화면 단위 매핑 표에도 처음부터 등장한 적이 없었고, 2026-09-03 폐지 작업에서 드러났습니다. [`doc-tracker/`](../doc-tracker/index.md) 「위험 진단」 참조.
...
**새 화면을 추가하면** — ① 그 화면을 터치포인트로 쓰는 단계가 여정 문서([`user-journey/`](../user-journey/))에 먼저 있어야 합니다 ② 기존 파일의 인라인 CSS 를 복사해 씁니다 — 임의의 색·radius·폰트를 새로 만들지 않습니다. 디자인 시스템이 바뀌면 모든 목업의 인라인 CSS 를 함께 갱신합니다 ③ 그 화면을 터치포인트로 쓰는 단계의 여정 페이지에 임베드하거나, 아직 이관 전이면 위 **이관 대기 원장**과 **화면별 매핑** 표에 행을 추가하고 상한을 올립니다 ④ 위 **Mockup index** 표에 행을 추가합니다 ⑤ [`doc-tracker/`](../doc-tracker/index.md)의 연결 매트릭스와 변경 이력, 허브 [`../index.html`](../index.html)의 링크를 함께 갱신합니다.

**여정을 이관하면** — 여정 페이지를 만들고, 흡수한 화면 파일을 삭제하고, 위 **Journeys** 표·**이관 대기 원장**·상한·허브·`doc-tracker/` 를 함께 갱신합니다. 체커가 넷의 불일치를 잡습니다.
